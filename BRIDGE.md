...
- Production (`nexus-labs`, `main`) confirmed live and current: Stark/JARVIS UI, dynamic agent registry wired into `/api/board`, Nex's Agent Board tools, Nex's branch-scoped build mode, `agent-lessons/`, SMS approvals (still needs Twilio env vars + webhook config before actually usable), searchable/tagged memory, a longer Nex chat window (24 messages), and now `listRepos()` for Nex — **PR #7 merged and live-verified**: asked Nex directly, he called it for real and returned the actual repo list (buehler-services, github-write-mcp, nex-test-repo, Nexus, nexus-labs, nexus-labs-sandbox).
- E2B/`run_sandbox` — verified fixed AND live-tested (`run_sandbox` executed a real command, correct output).
- **Every capability gap and stale-PR issue raised tonight is now closed.** This was a full audit-and-fix pass before any new epic work — worth reading the LOG below if picking this up cold.
- A second, independent Claude Code session (separate from this chat) proved itself tonight as a genuine board citizen — read BRIDGE.md + read_board on its own, found them in agreement, picked up and shipped the `list_repos` fix correctly, including catching and correcting its own mis-claimed task before continuing.
...
Read this file first, then `agent-lessons/`, before writing new code. With the fix-everything pass complete, the real next piece is **epic task 02, the dispatcher** — routes by required_capabilities/risk_class/approval_state to an available agent, idempotent claim/lease so duplicate triggers don't double-work. Builds on `lib/taskEnvelope.js`, so merging PR #2 (sandbox) first unblocks it. Codex: there's an unreviewed pricing thesis from you in the LOG below (platform subscription vs. BYOAI/BYOK vs. managed credits vs. outcome-based vs. hybrid) — worth picking that thread back up whenever it's relevant.
...
- A Claude Code session and this chat are separate agents that happen to share a model — both are expected to read/write BRIDGE.md and the board independently, same as Codex does. Don't assume "Claude" in the LOG always means this specific chat.

## LOG

- [2026-09-02] [CLAUDE] — PR #7 (`list_repos` for Nex) merged by Mr. Lopez, then live-verified: asked Nex directly, he called the real tool and returned the actual repo list, not a claim. That closes every item from tonight's fix-everything pass — E2B, PR #3, PR #1→#5, and now this. Nothing known-broken remains.
- [2026-09-02] [CLAUDE] — Second live Claude Code session came online, confirmed BRIDGE.md and read_board agree (wiring test passed), then picked up the `list_repos` gap Nex flagged. Corrected an early board mistake (claimed the wrong task id, released it back to idle) before creating the right one. Shipped `listRepos()` in `lib/github.js` + wiring in `lib/nexBrain.js`, verified with `node --check`, the existing regression suite, and a local mocked-fetch smoke test.
- [2026-09-01] [CLAUDE] — Fix-before-new-work audit: live-verified the E2B fix, confirmed the `list_repos` gap, and discovered PR #1 (searchable memory) and PR #3 (SMS) were fully built/tested but never merged. PR #3 merged. PR #1 had gone stale against `nexBrain.js` changes — rebuilt clean as PR #5, tests re-verified locally before opening. Added rules 11–12 so "PR exists" stops getting mistaken for "shipped."
