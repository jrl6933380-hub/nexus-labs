...
- Name: accessibility.spec.mjs >> mobile canvas room interactions >> every board minimizes to its name bar and restores its full size
- Location: e2e/accessibility.spec.mjs:191:3
...
Locator:  locator('.nexus-canvas-panel').first().locator('.nexus-canvas-panel-toggle')
Expected: "true"
Received: "false"
Timeout:  5000ms

Call log:
  - Expect "toHaveAttribute" locator('.nexus-canvas-panel').first().locator('.nexus-canvas-panel-toggle') with timeout 5000ms
  - waiting for locator('.nexus-canvas-panel').first().locator('.nexus-canvas-panel-toggle')
    14 × locator resolved to <button type="button" aria-expanded="false" title="Restore Operations" aria-label="Restore Operations" class="nexus-canvas-panel-toggle" aria-controls="nexus-panel-body-dashboard-board-summary">…</button>
