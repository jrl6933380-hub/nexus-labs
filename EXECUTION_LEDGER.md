...
Status: implemented for Nex tool dispatch and Board API consumers
Branch: test/execution-ledger
Storage: the existing server-side Redis connection
...
5. A terminal event also refreshes the run's resume pointer. Explicit
   `checkpoint` records cover longer pauses, handoffs, and cutoffs detected by
   the caller.
...
The Board API exposes actions for:

- `start_execution`
- `finish_execution`
- `checkpoint_execution`
- `get_execution_resume`
...
1. Done: ledger storage/helper with bounded redaction and a 200-event cap.
2. Done: lifecycle and checkpoint actions on the consolidated Board endpoint.
3. Done: Nex tool dispatch emits start plus terminal events. Ledger failures are
   non-fatal so observability cannot break the tool being observed.
4. Done: every terminal event refreshes a readable resume pointer; callers can
   write richer explicit checkpoints before a pause or handoff.
5. Deferred to Mission Control task 08: render activity rows and resume state.
6. Done: tests cover normal lifecycle, cut-off checkpoint/resume, secret
   redaction, malformed records, and unknown-result write recovery.
