...
export async function searchMemories(query, limit = SEARCH_LIMIT, options = {}) {
  const memories = await listMemories();
  return rankMemories(memories, query, limit, options);
}

export function rankMemories(memories, query, limit = SEARCH_LIMIT, options = {}) {
  const queryText = String(query || '').toLowerCase();
  const queryTerms = [...new Set(tokenize(queryText))];
  const safeLimit = Math.max(1, Math.min(Number(limit) || SEARCH_LIMIT, 20));

  const targetAgent = String(options.agent || '').toLowerCase();
  const requestedProject = String(options.project || '').toLowerCase();
  const requestedTopic = String(options.topic || '').toLowerCase();
  const required = (Array.isArray(memories) ? memories : [])
    .filter((memory) => (memory.status || ACTIVE_STATUS) === ACTIVE_STATUS)
    .filter((memory) => memory.category === 'for_claude' && (!targetAgent || targetAgent === 'claude'))
  ...
  const ranked = (Array.isArray(memories) ? memories : [])
    ...
    .filter((memory) => !(targetAgent === 'nex' && memory.category === 'for_claude'))
    .map((memory) => {
      ...
      const memoryProject = String(memory.project || '').toLowerCase();
      const memoryTopic = String(memory.topic || '').toLowerCase();
      if (requestedProject && memoryProject === requestedProject) score += 8;
      if (requestedTopic && memoryTopic === requestedTopic) score += 6;
      if (requestedProject && memory.scope === 'project' && memoryProject && memoryProject !== requestedProject) score -= 4;
