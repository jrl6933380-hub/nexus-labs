...
export async function checkpointExecution({ run_id, task_id = null, agent, state = 'paused', last_completed_tool = null, last_result = '', next_safe_action, working_branch = null, files_touched = [], blocker = null, perception_digest = null, scope_hash = null }) {
  run_id = requireText(run_id, 'run_id');
  agent = requireText(agent, 'agent');
  ...
  const pointer = clean({
    ...
    last_result, next_safe_action, working_branch, files_touched, blocker, perception_digest, scope_hash, updated_at: Date.now(),
  });
  await Promise.all([
    redisCommand(['HSET', POINTERS_KEY, run_id, JSON.stringify(pointer)]),
