...
export async function checkpointExecution({ run_id, task_id = null, agent, state = 'paused', last_completed_tool = null, last_result = '', next_safe_action, working_branch = null, files_touched = [], blocker = null, perception_digest = null, scope_hash = null, goal = '', current_step = '', acceptance_conditions = [], known_facts = [], loaded_capabilities = [], searched_capabilities = [], missing_evidence = [], tool_calls = 0, model_steps = 0, completion_replans = 0, failure_counts = [], blocked_tool_calls = [], reasoning_started_at = null, last_action_failed = false, record_event = true }) {
  run_id = requireText(run_id, 'run_id');
  agent = requireText(agent, 'agent');
  ...
  const pointer = clean({
    ...
    last_result, next_safe_action, working_branch, files_touched, blocker, perception_digest, scope_hash,
    goal, current_step, acceptance_conditions, known_facts, loaded_capabilities, searched_capabilities,
    missing_evidence, tool_calls, model_steps, completion_replans, failure_counts, blocked_tool_calls,
    reasoning_started_at, last_action_failed, updated_at: Date.now(),
  });
  const writes = [redisCommand(['HSET', POINTERS_KEY, run_id, JSON.stringify(pointer)])];
  if (record_event) writes.push(storeEvent({ ...pointer, type: 'checkpoint', event_id: id('event') }));
