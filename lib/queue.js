...
import { createOrUpdateFile, deleteFile, createRepo, deleteRepo, commitFiles, mergePullRequest } from './github.js';
import { deleteTask } from './board.js';
import { sendSms } from './sms.js';
import { executeBillingAction } from './stripeAdmin.js';
...
function executeQueuedItem(item) {
  ...
  if (item.tool === 'delete_board_task') {
    return deleteTask(item.input);
  }
  if (item.tool === 'merge_pull_request') {
    return mergePullRequest(item.input);
  }
  if (item.tool === 'stripe_billing_action') {
    return executeBillingAction(item.input);
  }
