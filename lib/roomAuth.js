...
import { getNexusOwner, NEXUS_OWNER_COOKIE } from './nexusOwnerAuth.js';
...
// Email confirmation state. Accounts created before this existed have
// no emailVerifiedAt field and so read as unverified — correct by
// default: the funded route should be closed until an address is
// actually confirmed, and the existing accounts can confirm through
// the same resend flow as anyone else.
export async function isEmailVerified(username) {
  if (!isValidUsername(username)) return false;
  const raw = await redisCommand(['HGET', USERS_KEY, username]);
  if (!raw) return false;
  try {
    return Boolean(JSON.parse(raw).emailVerifiedAt);
  } catch {
    return false;
  }
}

// Read-modify-write against a single hash field. Two confirmations of
// the same account can only ever write the same shape, and the token
// itself is single-use, so the benign race here is a repeated
// timestamp rather than a lost credential.
export async function markEmailVerified(username) {
  if (!isValidUsername(username)) return false;
  const raw = await redisCommand(['HGET', USERS_KEY, username]);
  if (!raw) return false;
  let record;
...
// Kill every live session belonging to one username.
//
// Sessions are stored token -> username with no reverse index, so this has
// to scan. That is acceptable at this scale (the same approach the Forge
// escalation store uses) and it is REQUIRED for account deletion: without
// it, deleting a user leaves their existing cookies valid for up to the
// 30-day session TTL, so a "deleted" account stays logged in and keeps
// passing getRequestUser.
export async function destroyAllSessionsForUser(username) {
  if (!username) return 0;
  const keys = await redisCommand(['KEYS', SESSION_PREFIX + '*']);
  if (!Array.isArray(keys) || !keys.length) return 0;
  const owners = await redisCommand(['MGET', ...keys]);
  const target = String(username).toLowerCase();
  const doomed = keys.filter((_key, index) => String(owners?.[index] || '').toLowerCase() === target);
  for (const key of doomed) await redisCommand(['DEL', key]);
  return doomed.length;
}

// Permanently remove an account: the user record, its Stripe reverse-index
// entry, and every live session. Returns what was actually removed.
//
// Deliberately has no operator check of its own — callers enforce policy
// (see lib/forgeAccounts.js deleteForgeAccount). This is the raw mechanism.
export async function deleteUser(username) {
  if (!isValidUsername(username)) throw new Error('Invalid username.');
  const raw = await redisCommand(['HGET', USERS_KEY, username]);
  if (!raw) throw new Error('No such user.');
  let record = {};
  try { record = JSON.parse(raw); } catch { /* malformed record still gets removed */ }

  // Drop the Stripe customer -> username mapping first. Leaving it behind
  // means a later cancellation webhook resolves to an account that no
  // longer exists and silently fails to downgrade anything.
  if (record.stripeCustomerId) {
    await redisCommand(['HDEL', STRIPE_CUSTOMERS_KEY, record.stripeCustomerId]);
  }
  const sessionsRevoked = await destroyAllSessionsForUser(username);
  await redisCommand(['HDEL', USERS_KEY, username]);
  ...
  return {
    ...
    hadPlan: record.plan || PLANS.FREE,
  };
}
...
export async function getRequestUser(req) {
  ...
  // The private Nexus owner may use Forge as its operator without maintaining
  // a second Forge login. Only a valid owner session grants this identity;
  // customer sessions above and guest access remain exactly as before.
  if (!cookies[NEXUS_OWNER_COOKIE] || !(await getNexusOwner(req))) return null;
  return process.env.NEXUS_FORGE_OWNER_USERNAME ||
    process.env.NEXUS_OPERATOR_USERNAMES?.split(',')[0]?.trim() || 'Mrlopez';
}
