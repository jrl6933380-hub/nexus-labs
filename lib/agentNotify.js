...
import { appendRecentConversation, primaryOperatorUsername } from './nexConversationStore.js';
...
export async function postChatCheckpoint({ title, status, note, owner }) {
  ...
  try {
    ...
    await appendRecentConversation(primaryOperatorUsername(), { role: 'assistant', content: line });
    return { posted: true };
  } catch (err) {
