...
function emptyCanvas(id, name) {
  return {
    ...
    backdrop_url: null,
    panels: {},
    // Free-form working notes for this venture, keyed by section id.
    // Server-side on purpose (not localStorage like the backdrop):
    // this is the venture's actual strategy/planning content, so Nex
    // needs to be able to read and contribute to it, and it must
    // survive switching devices.
    sections: {},
    created_at: Date.now(),
    updated_at: null,
  };
...
// Writes one working-notes section. Creating and updating are the
// same operation deliberately -- the client doesn't have to know
// whether a section already exists, which keeps autosave simple and
// idempotent.
export async function setCanvasSection({ canvas_id, id, title, body }) {
  ...
  state.sections[sectionId] = {
    id: sectionId,
    title: String(title ?? existing.title ?? sectionId).slice(0, 120),
    // Generous cap: this is meant to hold real strategy write-ups,
    // not one-line labels. Still bounded so a runaway client can't
    // push unbounded data into shared state.
    body: String(body ?? existing.body ?? '').slice(0, 20000),
...
export async function deleteCanvasSection({ canvas_id, id }) {
  const sectionId = String(id || '').trim();
  if (!sectionId) throw new Error('section id is required');
  const state = await getCanvasState(canvas_id);
  if (state.sections) delete state.sections[sectionId];
  return saveCanvasState(state);
}
...
// Renames a venture (or any non-dashboard canvas). This is the whole
// rename operation -- the name lives only on the canvas record, and
// every place that displays it (the home-screen tile via
// lib/venturesOverview.js, the canvas page's own title bar) reads
// straight from here, so one write updates both.
export async function renameCanvas({ canvas_id, name }) {
  const id = normalizeId(canvas_id);
  if (id === DEFAULT_CANVAS_ID) throw new Error('The dashboard canvas cannot be renamed');
  const trimmed = String(name || '').trim();
  if (!trimmed) throw new Error('name is required');
  const state = await getCanvasState(id);
  state.name = trimmed.slice(0, 120);
  return saveCanvasState(state);
}
