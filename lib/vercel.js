...
export async function checkDeploymentStatus({ owner, repo, target } = {}) {
  if (!owner || !repo) throw new Error('checkDeploymentStatus requires owner and repo.');
...
export async function linkRepoToVercel({ name, owner, repo, envVars = [] }) {
  if (!VERCEL_TOKEN) {
  ...
  try {
    data = await vercelRequest('/v9/projects', {
      ...
      body: JSON.stringify({
        ...
        ...(envVars.length
          ? { environmentVariables: envVars.map((e) => ({ key: e.key, value: e.value, target: ['production', 'preview'], type: 'encrypted' })) }
          : {}),
      }),
    });
  } catch (err) {
...
// Customer sites are deployed as their own Vercel projects, named by
// slugForProject() in api/room-publish.js: always `room-<user>-<project>`.
// Deleting the PROJECT (rather than individual deployments) is what
// actually takes the site offline and frees the URL.
//
// The prefix guard below is the important part. This function can delete a
// whole Vercel project permanently, so it refuses any name that isn't a
// customer room slug — `nexus-labs` itself, `github-write-mcp`, or anything
// else in the account can never be deleted through this path no matter what
// a caller passes in. If the slug format in room-publish.js ever changes,
// this guard has to change with it, deliberately.
const ROOM_SITE_NAME = /^room-[a-z0-9-]{1,84}$/;

export async function deleteStaticSite({ projectName, request = vercelRequest } = {}) {
  if (!VERCEL_TOKEN) {
    return { deleted: false, reason: 'No Vercel token configured (checked VERCEL_TOKEN and NEXS_TOKEN)' };
  }
  if (!projectName || !ROOM_SITE_NAME.test(projectName)) {
    // Deliberately a throw, not a soft return: being asked to delete a
    // non-room project means a caller is wrong about something, and the
    // safe response is to fail loudly rather than proceed.
    throw new Error('deleteStaticSite refuses any project that is not a customer room site (room-<user>-<project>).');
  }
  try {
    await request(`/v9/projects/${encodeURIComponent(projectName)}`, { method: 'DELETE' });
    return { deleted: true, projectName };
  } catch (err) {
    // A site that is already gone is a success from the caller's point of
    // view — the goal is "not live anymore", and it isn't.
    if (/\(404\)/.test(err.message)) {
      return { deleted: true, projectName, alreadyGone: true };
    }
    console.error('deleteStaticSite failed:', err.message);
