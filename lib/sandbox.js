...
export async function testCode({ language, code, timeoutMs } = {}) {
  assertAvailable();
  if (!code) throw new Error('No code provided to execute.');
  if (!['javascript', 'python'].includes(language)) throw new Error('language must be javascript or python.');
  const lifetime = Math.min(Math.max(Number(timeoutMs) || 30_000, 1_000), MAX_TIMEOUT_MS);
  const sandbox = await Sandbox.create({ timeoutMs: lifetime });
  try {
    ...
    const result = await sandbox.commands.run(command);
    return { language, stdout: result.stdout, stderr: result.stderr, exitCode: result.exitCode };
  } finally {
    await sandbox.kill();
  }
}
