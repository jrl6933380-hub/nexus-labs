...
export async function testCode({ language, code, timeoutMs } = {}) {
  if (!['python', 'javascript'].includes(language)) throw new Error('language must be python or javascript.');
  ...
  const result = await runInSandbox({
    commands: [`printf '%s' '${encoded}' | base64 -d > ${filename}`, command],
    timeoutMs,
  });
  const execution = result.results[1];
  return { language, stdout: execution.stdout, stderr: execution.stderr, exitCode: execution.exitCode };
}
