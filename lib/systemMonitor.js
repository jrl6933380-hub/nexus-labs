...
import { postChatCheckpoint } from './agentNotify.js';
...
const cleanupService = createCleanupService({
  readBoard,
  // Mirrors the cleaner's own board post into the same chat thread
  // Justin reads — this is the whole answer to "can Nex notice things
  // without being told": the existing 4s board poll already wakes
  // this sweep up, cleanupService already dedupes so it only posts on
  // a real new finding, and this just rides along on that, at zero
  // extra cost (no model call — postMessage firing IS the signal).
  postMessage: async (msg) => {
    const result = await postMessage(msg);
    postChatCheckpoint({
      title: 'System check',
      status: 'blocked',
      note: typeof msg?.message === 'string' ? msg.message : String(msg?.message ?? ''),
      owner: msg?.from || 'cleaner',
    }).catch(() => {});
    return result;
  },
