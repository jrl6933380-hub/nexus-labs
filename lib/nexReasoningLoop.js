...
const DEFAULT_BUDGETS = Object.freeze({
  ...
  maxElapsedMs: 170_000,
});

// Deep/complex runs (per lib/nexCognitiveController.js's planCognitiveRun
// complexity score, or an explicit 'crew' mode) get a materially larger
// window than routine chat/simple work, without touching the hard clamps
// enforced in normalizeBudgets below. Explicit caller-supplied budgets
// (toolContext.reasoningBudgets) still win over this when present -- this
// only changes what the *default* is when nothing explicit was passed.
const COMPLEXITY_BUDGETS = Object.freeze({
  simple: Object.freeze({ maxModelSteps: 14, maxToolCalls: 28, maxToolSearches: 5, maxElapsedMs: 170_000 }),
  moderate: Object.freeze({ maxModelSteps: 18, maxToolCalls: 36, maxToolSearches: 6, maxElapsedMs: 210_000 }),
  complex: Object.freeze({ maxModelSteps: 26, maxToolCalls: 60, maxToolSearches: 9, maxElapsedMs: 280_000 }),
});

function budgetsForPlan(plan = {}) {
  const key = plan.mode === 'crew' ? 'complex' : plan.complexity;
  return Object.hasOwn(COMPLEXITY_BUDGETS, key) ? COMPLEXITY_BUDGETS[key] : {};
}
...
function normalizeBudgets(input = {}) {
  return Object.freeze({
    maxModelSteps: boundedInt(input.maxModelSteps, DEFAULT_BUDGETS.maxModelSteps, 2, 30),
    maxToolCalls: boundedInt(input.maxToolCalls, DEFAULT_BUDGETS.maxToolCalls, 1, 80),
    maxToolSearches: boundedInt(input.maxToolSearches, DEFAULT_BUDGETS.maxToolSearches, 1, 12),
    ...
    maxElapsedMs: boundedInt(input.maxElapsedMs, DEFAULT_BUDGETS.maxElapsedMs, 5_000, 300_000),
  });
}
...
export function createReasoningState({ message, plan = {}, runState = {}, budgets = {}, now = Date.now() } = {}) {
  ...
  return {
    ...
    // Explicit caller-supplied budgets (toolContext.reasoningBudgets) win
    // field-by-field; anything not explicitly set falls back to the
    // complexity tier for this plan (see COMPLEXITY_BUDGETS above), and
    // only then to the flat DEFAULT_BUDGETS.
    budgets: normalizeBudgets({ ...budgetsForPlan(plan), ...budgets }),
  };
}
...
export function registerReasoningToolCall(state, block, now = Date.now()) {
  ...
  if (state.toolCalls >= state.budgets.maxToolCalls) {
    state.status = 'waiting';
    state.blocker = 'tool_call_budget_exhausted';
    return { allowed: false, reason: state.blocker };
  }
  state.toolCalls += 1;
