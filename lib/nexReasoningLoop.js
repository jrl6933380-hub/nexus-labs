...
export function createReasoningState({ message, plan = {}, runState = {}, budgets = {}, now = Date.now() } = {}) {
  ...
  return {
    ...
    toolCalls: Math.max(0, Number(runState.toolCalls) || 0),
    modelSteps: Math.max(0, Number(runState.modelSteps) || 0),
    completionReplans: Math.max(0, Number(runState.completionReplans) || 0),
    ...
    startedAt: Number(runState.startedAt) > 0 ? Number(runState.startedAt) : now,
...
export function registerToolSearch(state, query, resolvedCategories = []) {
  ...
  if (state.searchedCapabilities.size >= state.budgets.maxToolSearches) {
    state.status = 'waiting';
...
export function formatReasoningState(state) {
  ...
  return [
    ...
    `Budget used: model steps ${state.modelSteps}/${state.budgets.maxModelSteps}; tool calls ${state.toolCalls}/${state.budgets.maxToolCalls}; searches ${state.searchedCapabilities.size}/${state.budgets.maxToolSearches}`,
