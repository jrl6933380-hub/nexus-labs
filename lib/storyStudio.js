...
function normalizePanelImage(value) {
  ...
  return {
    ...
    letteringReviewPasses: Math.max(0, Math.min(2, Number(value?.letteringReviewPasses) || 0)),
...
export function prepareBasicComicPlan(value) {
  ...
  comic.panels = comic.panels.map((panel) => ({
    ...panel,
    dialogue: panel.dialogue.slice(0, 2).map((entry) => ({
      ...entry,
...
export const __internals = { PROJECT_LIMIT, PROJECT_ID_RE, MAX_SOURCE_LENGTH, MAX_PANELS, MAX_CHARACTERS };
