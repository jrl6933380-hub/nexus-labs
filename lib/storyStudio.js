...
function normalizePanelImage(value) {
  ...
  return {
    ...
    setIssues:cleanStringList(value?.setIssues, 6, 64)
      .filter((issue) => ['generated_text','blank_lettering_box','no_safe_lettering_space','unauthorized_character','inspection_failed'].includes(issue)),
    ...
    compositeReviewPasses: Math.max(0, Math.min(2, Number(value?.compositeReviewPasses) || 0)),
    compositeReviewedAt: Math.max(0, Number(value?.compositeReviewedAt) || 0),
    needsCompositeReview:value?.needsCompositeReview === true,
    compositeIssues:cleanStringList(value?.compositeIssues,10,64),
  };
}
