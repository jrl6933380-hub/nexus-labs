// /lib/nex/tools/agentUtility.js
// Final slice of the nexBrain.js TOOLS-array extraction: the last
// schemas that were still declared inline, sitting contiguously at the
// end of the array after ...ROOMS_TOOLS. Pure move — schemas only,
...
//
// Split into five exported groups by what they actually do (agent
// logs, model delegation, code vault, client launch, and the
// ask_user_question pause mechanic) rather than one catch-all array.
// The groups are spread back in this exact order at the same single
// position the inline block occupied, so the resulting TOOLS array is
// byte-for-byte the same list in the same order.

export const AGENT_LOG_TOOLS = [
  {
    name: 'log_exchange',
...
export const CLIENT_LAUNCH_TOOLS = [
  {
