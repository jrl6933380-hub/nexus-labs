...
export async function deleteTask({ id }) {
  ...
  await redisCommand(['HDEL', TASKS_KEY, id]);
...
function clampReadLimit(value, fallback, max) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(0, Math.min(Math.floor(parsed), max));
}

export function selectBoardRead({ tasks = [], messages = [] }, options = {}) {
  const view = ['all', 'active', 'messages'].includes(options.view) ? options.view : 'all';
  const query = String(options.query || '').trim().toLowerCase();
  const from = String(options.from || '').trim().toLowerCase();
  const status = String(options.status || '').trim();
  const taskLimit = clampReadLimit(options.task_limit, tasks.length, 200);
  const messageLimit = clampReadLimit(options.message_limit, messages.length, MESSAGE_LIMIT);
  const matchesQuery = (value) => !query || String(value || '').toLowerCase().includes(query);

  let selectedTasks = view === 'messages' ? [] : tasks.filter((task) => {
    if (view === 'active' && task.status === 'complete') return false;
    if (status && task.status !== status) return false;
    return matchesQuery([task.title, task.description, task.last_note, task.result, task.owner].filter(Boolean).join(' '));
  });
  let selectedMessages = messages.filter((entry) => {
    if (from && String(entry.from || '').toLowerCase() !== from) return false;
    return matchesQuery(entry.message);
  });

  selectedTasks = selectedTasks.slice(-taskLimit);
  selectedMessages = selectedMessages.slice(0, messageLimit);

  return {
    // Messages intentionally come first. Tool relays cap oversized payloads,
    // and recent inter-agent notes must not disappear behind task history.
    ...
    page: {
      ...
      from: from || null,
      status: status || null,
      returned_tasks: selectedTasks.length,
      returned_messages: selectedMessages.length,
...
export async function readBoard(options) {
  ...
  if (options === undefined) return { tasks, messages };
  return selectBoardRead({ tasks, messages }, options);
}
