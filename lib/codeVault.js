...
/**
 * Normalize a Redis HGETALL result into a plain {field: value} object.
 * Upstash's REST API returns HGETALL as a flat array of alternating
 * field/value strings (["field1","value1","field2","value2", ...]),
 * not a nested object — treating that array as already-keyed meant
 * field NAMES (like "nexus:vault:blueprint:x") got passed to
 * JSON.parse as if they were values, which is exactly what broke
 * searchVault/listVaultItems the first time this ran against real
 * Redis instead of the in-memory test mock (which never exercised
 * this shape). Exported standalone so it's unit-testable without a
