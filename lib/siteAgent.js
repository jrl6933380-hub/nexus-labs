...
// Reverse index: Stripe Subscription id -> projectId. A cancellation
// or past-due event only carries the subscription id, and a single
// account can have its main plan subscription PLUS several Site
// Agent subscriptions (one per project) — this is what lets the
// webhook tell those apart and disable only the right project instead
// of guessing or downgrading the whole account by mistake.
...
// Per-visitor abuse guard on the PUBLIC chat endpoint, checked before
// the monthly reply cap and before any model call. The monthly cap
// bounds total cost for the month; this bounds how fast one visitor
// (by IP) can burn through it. Fails OPEN on a Redis error rather
// than blocking every visitor if the store hiccups — unlike the
// reply cap itself, this is an abuse guard, not the primary billing
// safety property, so availability wins the tradeoff here.
...
export async function checkRateLimit(projectId, ip) {
  ...
  try {
    ...
    console.error('siteAgent checkRateLimit failed, failing open:', err.message);
    return { allowed: true };
  }
}
