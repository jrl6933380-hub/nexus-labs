...
import { createUser, setUserPlan, listUsers, deleteUser, isOperatorUser, SECURITY_QUESTIONS, PLANS } from './roomAuth.js';
...
// Permanent account deletion. Irreversible: there is no undo and no backup
// of the record, so the guardrails below matter more than the convenience.
//
// Two things are refused outright rather than made overridable:
//
//  1. Operator accounts. Operator status is environment-derived, so deleting
//     the record would NOT remove their operator rights — it would just
//     destroy the account they sign in with, locking the owner out of their
//     own admin surface while leaving the privilege dangling. There is no
//     good reason to do this from here.
//
//  2. Accounts with billing on record, unless the caller explicitly passes
//     acknowledgeBilling. A Stripe subscription lives on Stripe, not here:
//     deleting the account does NOT cancel it, so the customer keeps being
//     charged for a login that no longer exists. The subscription should be
//     cancelled first through the approved billing actions. The flag exists
//     so it is possible when genuinely intended, never accidental.
export async function deleteForgeAccount({ username, acknowledgeBilling = false } = {}) {
  if (!username || typeof username !== 'string') throw new Error('username is required.');

  const found = await findForgeAccount(username);
  if (!found.exists) throw new Error('No such account.');
  const account = found.account;

  if (account.operator) {
    throw new Error(
      'Refusing to delete an operator account. Operator status comes from NEXUS_OPERATOR_USERNAMES, '
      + 'so deleting the record would remove their login without removing their privileges. '
      + 'Remove them from that environment variable instead.',
    );
  }

  if (account.hasBillingOnRecord && !acknowledgeBilling) {
    throw new Error(
      `${account.username} has billing on record. Deleting the account does NOT cancel their Stripe `
      + 'subscription — they would keep being charged with no way to sign in. Cancel the subscription '
      + 'first, or pass acknowledgeBilling if that has already been handled.',
    );
  }

  const result = await deleteUser(account.username);
  return { ...result, hadForgeRole: account.forgeRole, hadBillingOnRecord: account.hasBillingOnRecord };
}
