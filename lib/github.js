...
export async function createRepo({ owner, name, description, private: isPrivate = false, envVars = [] }) {
  ...
  const vercel = await linkRepoToVercel({ name: data.name, owner: data.owner.login, repo: data.name, envVars });
...
// Secret-pattern scan on the actual added lines of a diff — same
// spirit as Replit Agent's pre-completion Semgrep pass: catch a
// hardcoded credential before it ever reaches a PR, not after. Kept
// to well-known, high-confidence patterns (real key/token formats)
// rather than broad heuristics, so it flags real leaks without
// nagging on every string that merely contains the word "key".
const SECRET_PATTERNS = [
  ...
  { name: 'Generic bearer/API key assignment', re: /(api[_-]?key|secret|token|bearer)\s*[:=]\s*['"][A-Za-z0-9_\-.]{20,}['"]/i },
];
...
export function findSecretLeaks(files = []) {
  ...
  for (const file of files) {
    ...
    // Only the added side of the diff — a secret being removed isn't a
    // new leak, and scanning full context lines would double-count.
    const addedLines = file.patch.split('\n').filter((line) => line.startsWith('+') && !line.startsWith('+++'));
    for (const line of addedLines) {
      for (const pattern of SECRET_PATTERNS) {
        if (pattern.re.test(line)) {
          findings.push({ path: file.filename, pattern: pattern.name });
          break; // one flag per line is enough, move to the next line
        }
      }
    }
  }
...
export async function inspectBranchDiff({ owner, repo, head, base }) {
  ...
  return {
    ...
    secret_leaks: findSecretLeaks(files),
  };
}
...
export async function createPullRequest({ owner, repo, title, head, base, body: prBody }) {
  ...
  if (inspection.secret_leaks.length) {
    // Never echo the matched text itself — only the file and pattern
    // type. The finding is useful; the actual secret value in a log or
    // error message would just be a second leak.
    const summary = inspection.secret_leaks.map((f) => `${f.path} (${f.pattern})`).join(', ');
    throw new Error(`PR_SAFETY_BLOCKED: possible hardcoded secret detected: ${summary}. Move it to an environment variable and remove it from the diff before opening a PR.`);
  }
...
export async function mergePullRequest({ owner, repo, pull_number, merge_method, commit_title, commit_message }) {
  const body = {
...
// The Checks API is what actually answers "did the CI/Actions workflow
// pass or fail" — a PR's mergeable_state (e.g. "unstable") only tells you
// SOMETHING isn't green yet, never which check or why. ref accepts a SHA,
// branch name, or tag per GitHub's docs.
export async function getCommitCheckStatus({ owner, repo, ref }) {
  const { ok, status, data } = await githubRequest(
    `/repos/${owner}/${repo}/commits/${encodeURIComponent(ref)}/check-runs?per_page=100`
  );
  if (!ok) {
    throw new Error(`GitHub API error fetching check runs for ${ref} (${status}): ${JSON.stringify(data).slice(0, 300)}`);
  }
  ...
  return {
    ...
    check_runs: runs.map((r) => ({
      ...
      conclusion: r.conclusion, // success | failure | cancelled | skipped | timed_out | action_required | neutral | null
