// /lib/dispatcher.js
// Approval-aware dispatcher with idempotent locks — epic task 02.
// PORTED FROM nexus-labs-sandbox into production, unchanged logic.
// Routes work by required capability, connection mode, availability,
// and cost tier; a dispatch requires approval when policy says so;
// idempotency keys and atomic claim/lease prevent double-dispatch.
...
export class MemoryDispatchStore {
  ...
  // Read-side counterpart to appendAudit — without this, every
  // dispatched/completed/failed event this dispatcher ever records is
  // write-only, which fails the epic's own success criterion ("every
  // run exposes evidence, cost, and an audit trail"). Most-recent-first,
  // matching how the Redis store's LPUSH + LRANGE naturally orders.
  async listAudit(limit = DEFAULT_AUDIT_LIMIT) {
...
export class RedisDispatchStore {
  ...
  // Read-side counterpart to appendAudit. LRANGE naturally returns
  // most-recent-first since appendAudit always LPUSHes onto the head.
  async listAudit(limit = DEFAULT_AUDIT_LIMIT) {
    const raw = await this.command(['LRANGE', `${this.prefix}:audit`, '0', String(Math.max(0, limit - 1))]);
...
export function createDispatcher({ store, listAgentsFn = listAgents, now = () => Date.now(),
  ...
  return {
    ...
    listAudit(limit) { return store.listAudit(limit); },
  };
}
