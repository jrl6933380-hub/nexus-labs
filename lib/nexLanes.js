...
// The real source of truth for the two contracts below is now
// nex-skills/grounding-contract/SKILL.md and nex-skills/write-contract/SKILL.md
// -- editing those files changes what every lane loads, no code review of
// this module required. This module reads them synchronously (contractFor
// is called from a non-async system-prompt build path) and parses out the
// instructions body. The FALLBACK_* strings below only exist so a
// missing/corrupt skill file degrades to known-good text instead of
// silently dropping the contract -- they are a safety net, not the primary
// copy, and should stay identical to the skill files' instructions.
const FRONTMATTER = /^---\n([\s\S]*?)\n---\n([\s\S]+)$/u;

export function readContractSkill(skillDir) {
  try {
    const skillPath = path.join(process.cwd(), 'nex-skills', skillDir, 'SKILL.md');
    const raw = fs.readFileSync(skillPath, 'utf8').replace(/\r\n/gu, '\n');
    const match = raw.match(FRONTMATTER);
    const instructions = match ? match[2].trim() : '';
    // Strip the trailing "zero enforcement power" disclaimer paragraph so
    // the exported string stays identical to the original inline contract
    // text that tests assert against.
    return instructions ? instructions.split('\n\nThis skill carries zero enforcement power')[0].trim() : null;
  } catch {
    return null;
  }
}
...
export const GROUNDING_CONTRACT = readContractSkill('grounding-contract') || [
...
export const WRITE_CONTRACT = readContractSkill('write-contract') || [
