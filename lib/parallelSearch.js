...
// language research goal instead. IMPORTANT: Parallel's live API
// (verified against the real MCP server, not just docs) requires BOTH
// objective AND search_queries on every call — it is not actually
// either/or despite how that reads. So if only one is given, derive
// the other from it rather than omitting it. max_results caps how
// many results come back.
export async function webSearch({ query, objective, max_results } = {}) {
  await ensureInitialized();
  const queries = query ? (Array.isArray(query) ? query : [query]) : (objective ? [objective] : []);
  if (queries.length === 0) throw new Error('web_search requires query or objective');
  const args = {
