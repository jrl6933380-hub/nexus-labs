...
import { listFiles, readFile, listRepos, createBranch, createPullRequest, searchCode, commitFiles, createOrUpdateFile, deleteFile, getDefaultBranch, readIssue, readPullRequest, listPullRequests, listPullRequestFiles } from './github.js';
...
export function compactModelHistory(history, { maxMessages = 12, maxContentChars = 3500 } = {}) {
  return (Array.isArray(history) ? history : []).slice(-maxMessages).map((entry) => ({
    ...entry,
    content: truncatePromptValue(entry.content, maxContentChars),
  }));
}
...
const TOOLS = [
  ...
  { name:'prepare_build_handoff',description:'Create the first pipe for a multi-step build: a compact context-to-build packet. Call before broad exploration or implementation. The returned packet is the builder pipe brief.',input_schema:{type:'object',properties:{goal:{type:'string'},owner:{type:'string'},repo:{type:'string'},paths:{type:'array',items:{type:'string'}},branch:{type:'string'},acceptance_criteria:{type:'array',items:{type:'string'}}},required:['goal','owner','repo']} },
...
async function callModel(tier, model, history, identityText, memoriesText, snapshotContext, liveWorkspaceContext, accumulatedUsage) {
      ...
      if (block.name === 'save_memory') {
        ...
        try {
          ...
          toolResults.push({
            type: 'tool_result',
            tool_use_id: block.id,
            content: `New venture canvas ready at ${url}. Linked Board task: ${task.id}. Canvas: ${JSON.stringify(canvas)}`,
          });
        } catch (err) {
...
// ============================================================
export async function askNex(message, history = [], forcedTier = null, clientContext = {}) {
  ...
  const modelHistory = compactModelHistory(updatedHistory);
  const tier = forcedTier && MODEL_TIERS[forcedTier] ? forcedTier : await classifyTier(message);
  const model = MODEL_TIERS[tier];
  try {
    const result = await callModel(tier, model, modelHistory, identityText, memoriesText, snapshotContext, liveWorkspaceContext);
    return {
      reply: result.reply,
      updatedHistory,
      model: result.model,
