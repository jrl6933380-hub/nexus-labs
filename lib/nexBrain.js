...
async function callClaude(model, history, identityText, memoriesText) {
  ...
  async function send(messages) {
    const response = await fetch(CLAUDE_ENDPOINT, {
      ...
      body: JSON.stringify({
        model,
        // Bumped from 2000 — that was too small to ever complete a full
        // file rewrite in one tool call (Nex hit this repeatedly trying
        // to patch his own code: the response kept getting cut off
        // mid-write with stop_reason "max_tokens", which looked like a
        // silent failure since there was no text block to show for it).
        max_tokens: 8192,
        system: systemPrompt,
        tools: TOOLS,
        messages,
  ...
  while (data.stop_reason === 'tool_use') {
    ...
    for (const block of toolUseBlocks) {
      if (block.name === 'save_memory') {
        ...
        // Guarantees every tool_use block always gets a matching
        // tool_result. Without this, an unrecognized tool name would
        // leave toolResults short — and if it were the ONLY block in
        // this round, toolResults would end up empty. Claude's API
        // rejects a user message with empty content outright, which is
        // the exact bug that crashed things earlier tonight. This closes
        // that gap for good, for any tool name, known or not.
