...
export const TOOLS = [
  ...
  },
  {
    ...
    description: "Pause what you're doing and ask Mr. Lopez a real question with a couple of tappable options — mid-task, not just at the start or end of a conversation. Use this the moment a genuine fork comes up while you're working: a naming/style choice, whether to also do an adjacent thing while you're already in there, which of two reasonable approaches to take. This actually stops and waits for his real answer before you continue — it is not a rhetorical question, so don't call it unless you're prepared to act differently depending on which option he picks. Say what you're already doing and why you're asking in your normal reply text alongside this call; the options ride next to that text as tappable choices. Keep it to one question and 2-4 short options at a time — this is for a real fork in the road, not a way to avoid making a reasonable default choice yourself.",
    input_schema: {
      type: 'object',
      properties: {
        question: { type: 'string', description: 'The actual question — short and specific.' },
        options: {
          type: 'array',
  ...
  while (data.stop_reason === 'tool_use') {
    ...
    for (const block of toolUseBlocks) {
      ...
      if (block.name === 'direct_story_actor') {
        ...
        if (question) pendingQuestion = { question, options };
        toolResults.push({
          type: 'tool_result',
          tool_use_id: block.id,
          content: question ? 'Question posed to Mr. Lopez — awaiting his reply before continuing.' : 'ask_user_question requires a non-empty question.',
          is_error: !question,
        });
      } else {
