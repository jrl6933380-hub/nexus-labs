...
export async function askNex(message, history = [], forcedTier = null, clientContext = {}, onBuildEvent = null, toolContext = {}) {
  ...
  const compiledContext = compileNexContext({ memories, liveWorkspaceContext, snapshot, cognitivePlan });
  try {
    const result = await callModel(tier, model, modelHistory, identityText, 'See the compiled context packet below.', '', compiledContext.text, undefined, onBuildEvent, { ...toolContext, cognitivePlan });
    return {
      reply: result.reply,
      updatedHistory,
      model: result.model,
