...
import { listFiles, readFile, createBranch, createPullRequest, searchCode, commitFiles, createOrUpdateFile, deleteFile, getDefaultBranch } from './github.js';
...
const TOOLS = [
  ...
  {
    name: 'create_branch',
    description: 'Create a new branch in a GitHub repo, branched off an existing branch. Executes immediately (no approval needed) since it never touches the live/default branch — it just makes a safe copy to work on. Use this first when starting real independent work: branch, then write to that branch freely with create_repo_file/update_repo_file/commit_repo_files (which execute immediately on a non-live branch), then open a pull request with create_pull_request so Mr. Lopez can review the actual diff before it goes live.',
    input_schema: {
      type: 'object',
      properties: {
        owner: { type: 'string' },
