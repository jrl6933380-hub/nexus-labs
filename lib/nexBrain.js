...
import { formatCognitiveDirective, planCognitiveRun, resolveCognitiveTier } from './nexCognitiveController.js';
...
// system status, exchange logging) plus tool_search itself are
...
// or its schema (see TOOLS above, untouched). It only changes whether
// that schema is IN the array sent to Anthropic for a given call.
// ============================================================
...
export const NEX_TOOL_CATEGORIES = {
  coding: {
    ...
    tools: [
      ...
      'read_issue', 'read_pull_request', 'list_pull_requests', 'create_branch',
...
const TOOL_SEARCH_SCHEMA = {
  name: 'tool_search',
  description: "Load a group of your own tools by category before using them — most of your tools are NOT in your list by default, only memory, board reading, reference links, system status, and exchange logging are. Call this the moment you realize a task needs GitHub/code work, a Vercel deployment check, a Stripe billing action, Hyperfocus/handoff/pipeline coordination, the Code Vault, the Crash Feed, rooms/ventures, delegating to another model, or Story Studio direction — pass the category key if you know it, or a few plain keywords describing what you need. Matched tools become directly callable in this same turn. Call with no confident match (or an empty query) to see the full category directory.",
  input_schema: {
    type: 'object',
    properties: {
...
function matchCategoriesKeyword(query) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return [];
  const entries = Object.entries(NEX_TOOL_CATEGORIES);

  // Exact or substring match against the category key or its label
  // wins outright — this is the common case (Nex passing "coding" or
  // "billing" directly).
  ...
  // Otherwise score each category by how many of its tool names or
  // descriptions contain any word from the query.
  const words = q.split(/\s+/).filter((w) => w.length > 2);
  if (!words.length) return [];
  const byName = new Map(TOOLS.map((t) => [t.name, t]));
  const scored = entries.map(([key, cat]) => {
    const haystack = cat.tools.map((name) => `${name} ${byName.get(name)?.description || ''}`).join(' ').toLowerCase();
...
function buildActiveTools(unlockedCategories) {
  ...
  for (const key of unlockedCategories) {
    (NEX_TOOL_CATEGORIES[key]?.tools || []).forEach((name) => unlockedNames.add(name));
  }
  const byName = new Map(TOOLS.map((t) => [t.name, t]));
  const coreTools = TOOLS.filter((t) => CORE_TOOL_NAMES.has(t.name));
  const unlockedTools = Array.from(unlockedNames).map((name) => byName.get(name)).filter(Boolean);
  return [TOOL_SEARCH_SCHEMA, ...coreTools, ...unlockedTools];
}
...
## Building a genuinely missing tool yourself: for a real capability gap you're confident is well-scoped and safe, you don't have to wait for Claude — you already have everything you need. Add it to lib/nexBrain.js the exact same way every existing tool got there: a schema in TOOLS, a category entry in NEX_TOOL_CATEGORIES (or CORE_TOOL_NAMES if it truly needs to be always-loaded), and a dispatch branch that calls real, already-tested logic — never invent a fake success path. This is still code review, not a live capability the moment you write it: tool_search a coding category, branch, write it, run_sandbox to verify it actually works, then open a pull request like any other change. Never merge it yourself, and never tell Mr. Lopez a new tool is "ready to use" until that PR is actually merged and deployed — a written tool and a live one are different things. Reach for this for a real, scoped gap; a save_memory "for_claude" note is still the right call for something bigger than you should take on solo (new external service integration, anything touching billing/auth, anything you're not confident is safe to build without a second opinion).`;
  ...
  const stableSystemText = [systemPrompt, getNexRuntimePolicy()].filter(Boolean).join('\n\n');
  const dynamicSystemText = [
    ...
    for (const block of toolUseBlocks) {
      ...
      if (!authorization.allowed) {
        ...
        try {
          ...
          const matched = await matchCategoriesAsync(query);
      ...
      if (runState) {
        ...
        if (shouldCheckpointNexRun(block, toolResults[resultIndex])) {
  ...
  const textBlock = data?.content?.find((block) => block.type === 'text');
  const reply = textBlock?.text || (pendingQuestion ? pendingQuestion.question : undefined);
  ...
  if (!reply) {
    ...
    if (model === MODEL_TIERS.cheap) {
      return callModel('standard', MODEL_TIERS.standard, history, identityText, memoriesText, snapshotContext, liveWorkspaceContext, usage, onBuildEvent, toolContext);
    }
    if (model === MODEL_TIERS.standard) {
      return callModel('heavy', MODEL_TIERS.heavy, history, identityText, memoriesText, snapshotContext, liveWorkspaceContext, usage, onBuildEvent, toolContext);
    }
  ...
  return {
    ...
    suggestedReplies,
    pendingApproval,
    completionReceipt: evidenceTracker.receipt(),
...
export async function askNex(message, history = [], forcedTier = null, clientContext = {}, onBuildEvent = null, toolContext = {}) {
  ...
  try {
    const result = await callModel(tier, model, modelHistory, identityText, 'See the compiled context packet below.', '', compiledContext.text, undefined, onBuildEvent, { ...toolContext, cognitivePlan, runState });
    return {
      reply: result.reply,
      updatedHistory,
      model: result.model,
