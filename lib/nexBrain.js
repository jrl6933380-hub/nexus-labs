...
import { listFiles, readFile, pageFileContent, listRepos, createBranch, createPullRequest, searchCode, commitFiles, createOrUpdateFile, deleteFile, getDefaultBranch, readIssue, readPullRequest, listPullRequests, listPullRequestFiles } from './github.js';
...
import { listVercelProjects, listVercelDeployments, getVercelBuildLogs } from './vercel.js';
...
function buildFeedbackLabel(tool) {
  return ({ create_branch: 'Creating branch', create_repo_file: 'Writing file', update_repo_file: 'Updating file', delete_repo_file: 'Removing file', commit_repo_files: 'Committing changes', run_sandbox: 'Running verification', create_pull_request: 'Opening pull request' })[tool] || null;
}

async function callModel(tier, model, history, identityText, memoriesText, snapshotContext, liveWorkspaceContext, accumulatedUsage, onBuildEvent = null) {
...
// ============================================================
export async function askNex(message, history = [], forcedTier = null, clientContext = {}, onBuildEvent = null) {
  ...
  try {
    const result = await callModel(tier, model, modelHistory, identityText, memoriesText, snapshotContext, liveWorkspaceContext, undefined, onBuildEvent);
    return {
      reply: result.reply,
      updatedHistory,
      model: result.model,
