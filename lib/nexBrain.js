...
async function callModel(tier, model, history, identityText, memoriesText, accumulatedUsage) {
  ...
  if (!reply) {
    ...
    if (model === MODEL_TIERS.cheap) {
      return callModel('standard', MODEL_TIERS.standard, history, identityText, memoriesText, usage);
    }
    if (model === MODEL_TIERS.standard) {
      return callModel('heavy', MODEL_TIERS.heavy, history, identityText, memoriesText, usage);
    }
...
export async function askNex(message, history = [], forcedTier = null) {
  ...
  try {
    const result = await callModel(tier, model, updatedHistory, identityText, memoriesText);
    return {
      reply: result.reply,
      updatedHistory,
      model: result.model,
