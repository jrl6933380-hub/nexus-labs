...
import { renderPinnedVisual } from './pinnedVisuals.js';
...
export function inferPreloadedToolCategories(message) {
  ...
  const matched = matchCategoriesKeyword(text);
  const visualWorkspaceIntent = /\b(draw|pin|render|visualize|diagram)\b/iu.test(text)
    || /\b(show|map|inspect|break(?:\s+it)?\s+down)\b[\s\S]{0,90}\b(section|system|flow|architecture|screen|canvas|workspace|upgrade)s?\b/iu.test(text);
  return visualWorkspaceIntent ? [...new Set(['rooms', ...matched])] : matched;
}

const EXPLICIT_ACTION = /\b(build|check|create|delete|deploy|draw|edit|execute|fix|inspect|list|merge|open|patch|pin|read|render|run|search|show|take me|test|update|visualize|write)\b/iu;
...
export function initialToolChoiceForRequest(message, categories = inferPreloadedToolCategories(message)) {
  ...
  if (available.has('rooms') && /\b(draw|pin|render|visualize)\b/iu.test(text)) {
    return { type: 'tool', name: 'render_visual' };
  }
  if (available.has('rooms') && /\b(show|map|inspect|break(?:\s+it)?\s+down)\b[\s\S]{0,90}\b(section|system|flow|architecture|screen|canvas|workspace|upgrade)s?\b/iu.test(text)) {
    return { type: 'tool', name: 'render_visual' };
  }
...
function buildFeedbackLabel(tool) {
  return ({ direct_story_actor:'Directing comic actor', create_branch: 'Creating branch', create_repo_file: 'Writing file', update_repo_file: 'Updating file', patch_repo_file: 'Patching file', delete_repo_file: 'Removing file', commit_repo_files: 'Committing changes', run_sandbox: 'Running verification', create_pull_request: 'Opening pull request', render_visual: 'Rendering pinned visual' })[tool] || `Using ${String(tool).replace(/_/g, ' ')}`;
}
...
async function callModel(tier, model, history, identityText, snapshotContext, liveWorkspaceContext, accumulatedUsage, onBuildEvent = null, toolContext = {}) {
  ...
  async function send(messages) {
    onBuildEvent?.({ type: 'stage', state: 'running', tool: 'reasoning', label: 'Reviewing context and choosing the next step' });
  ...
  while (true) {
    ...
    for (const block of toolUseBlocks) {
      ...
      if (!reasoningAuthorization.allowed) {
        ...
        try {
          const visual = await renderPinnedVisual(block.input);
          toolResults.push({
            type: 'tool_result',
            tool_use_id: block.id,
            content: JSON.stringify({
              ...
              instruction: visual.rendered
                ? 'The visual is now displayed in that room’s pinned panel.'
                : 'The panel is locked, so the visual was saved to history without replacing the active visual.',
            }),
          });
        } catch (err) {
          console.error('render_visual tool failed:', err.message);
          toolResults.push({
            type: 'tool_result',
            tool_use_id: block.id,
            content: `Failed to render visual: ${err.message}`,
            is_error: true,
          });
        }
