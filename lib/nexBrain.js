...
async function callModel(tier, model, history, identityText, snapshotContext, liveWorkspaceContext, accumulatedUsage, onBuildEvent = null, toolContext = {}) {
  ...
  while (true) {
    ...
    for (const block of toolUseBlocks) {
      ...
      const reasoningAuthorization = reasoningState
      ...
      if (!reasoningAuthorization.allowed) {
        ...
        try {
          ...
          const searchAuthorization = reasoningState ? registerToolSearch(reasoningState, query) : { allowed: true };
          if (!searchAuthorization.allowed) {
            throw new Error(`Search blocked by reasoning controller: ${searchAuthorization.reason}`);
          }
      ...
      if (reasoningResult.blocked) {
        forcedStopReply = `I stopped this run after the same ${block.name} failure repeated twice. I saved the checkpoint instead of trying a third blind variation. The latest result was: ${String(toolResults[resultIndex]?.content || '').slice(0, 500)}`;
      }
  ...
  if (runState && reasoningState) {
    ...
    try { await persistNexRunState(runState); } catch (error) { console.error('Nex final run checkpoint failed:', error.message); }
  }
  const textBlock = data?.content?.find((block) => block.type === 'text');
