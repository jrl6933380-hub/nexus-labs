...
export const TOOLS = [
  ...
  {
    name: 'read_board',
    description: "Read the shared Agent Board without losing newest messages to oversized task history. Defaults to active tasks plus the 20 newest messages. Use view=messages with query/from to find an exact inter-agent note; use view=all only when completed history is actually needed. Executes immediately.",
    input_schema: {
      type: 'object',
      properties: {
        view: { type: 'string', enum: ['active', 'all', 'messages'], description: 'active (default), all task history, or messages only.' },
        query: { type: 'string', description: 'Case-insensitive search across task text and message bodies.' },
        from: { type: 'string', description: 'Only messages from this agent, e.g. claude.' },
        status: { type: 'string', description: 'Only tasks with this exact status.' },
        task_limit: { type: 'number', description: 'Maximum matching tasks returned (0-200).' },
        message_limit: { type: 'number', description: 'Maximum matching newest messages returned (0-50).' },
      },
    },
  },
  {
  ...
  while (data.stop_reason === 'tool_use') {
    ...
    for (const block of toolUseBlocks) {
      ...
      if (block.name === 'direct_story_actor') {
        ...
        try {
          const board = await readBoard({ view: 'active', task_limit: 40, message_limit: 20, ...block.input });
