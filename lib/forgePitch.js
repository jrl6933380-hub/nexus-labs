...
function weaknessLines(lead) {
  const lines = [];
  // The sharpest pain point available: real proof people already like
  // them, undercut by having nowhere online to show it. Only fires on
  // enriched leads (lib/forgePlaces.js) with a decent rating/review
  // count and no website Google has on file — never a guess.
  const hasNoWebsite = !lead.placesWebsite && (!lead.website || lead.websiteStatus === 'absent');
  if (typeof lead.rating === 'number' && typeof lead.reviewCount === 'number' && lead.reviewCount >= 5 && lead.rating >= 4 && hasNoWebsite) {
    lines.push(`You've got a ${lead.rating}-star rating from ${lead.reviewCount} reviews — but there's no website showing that off when someone searches for you.`);
  }
