...
// Read env lazily rather than capturing it at module load. Module-level
// capture meant the value was frozen at first import: anything importing
// this module before the env was populated got a permanently unconfigured
// client, and it could not be exercised in tests at all (every case fell
// through to "No NEON_API_KEY configured" regardless of what the test set).
...
export async function deleteNeonDatabase({ project_id } = {}) {
  const apiKey = neonApiKey();
  if (!apiKey) return { deleted: false, reason: 'No NEON_API_KEY configured.' };
