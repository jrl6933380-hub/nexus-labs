...
export async function openBuildStream({ username, body, signal, isEdit = false }) {
  ...
  // Metered before the first provider call. A build is the expensive kind of
  // request, so this is the reservation that matters most — and it is taken
  // before `fetchWithRotation` below, not after, so a refused build costs
  // nothing at all.
  ...
  let firstResponse;
  try {
    firstResponse = await fetchWithRotation(baseMessages);
  } catch (error) {
    // Nothing streamed, so nothing was spent on a completed call. Release the
    // hold immediately rather than letting it sit for the TTL and wrongly
    // block the customer's next attempt.
    await reservation.settle({ success: false });
    throw error;
  }
  ...
  return {
    response: { body: continuedStream(firstResponse.body, {
      ...
      // Settlement is deferred to whenever the stream actually ends — clean
      // close, error, or customer cancel. continuedStream calls this exactly
      // once on each of those paths, and settle() is idempotent besides.
      onFinished: ({ success }) => { reservation.settle({ success }).catch(() => {}); },
    }) },
    provider,
    model: currentModel,
    byo: true,
