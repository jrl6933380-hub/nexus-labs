 ...
 */
export function continuedStream(firstBody, { openRound, baseMessages, maxRounds = MAX_CONTINUATION_ROUNDS, continueOnIncomplete = false, stallTimeoutMs = 25_000, maxStallRetries = 2 }) {
  const encoder = new TextEncoder();
      ...
      try {
        ...
        if (result.stalled) {
          if (round === 1 && accumulated === '' && stallRetries < maxStallRetries) {
            stallRetries += 1;
            console.log(`forge build: round 1 stalled with no visible output, retrying with a fresh model pick (attempt ${stallRetries + 1})`);
            const next = await openRound(baseMessages);
...
export async function openBuildStream({ username, body, signal, isEdit = false }) {
  ...
  return {
    response: { body: continuedStream(firstResponse.body, {
      openRound, baseMessages, maxRounds: budget.maxRounds, continueOnIncomplete: !isEdit,
    }) },
