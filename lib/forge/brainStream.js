 ...
 */
export function continuedStream(firstBody, { openRound, baseMessages, maxRounds = MAX_CONTINUATION_ROUNDS }) {
  const encoder = new TextEncoder();
...
export async function openBuildStream({ username, body, signal }) {
  ...
  return {
    response: { body: continuedStream(firstResponse.body, { openRound, baseMessages, maxRounds: budget.maxRounds }) },
