...
export async function savePendingAuthorization(username, { state, codeVerifier, provider, tier, returnTo }) {
  if (!username || !state) throw new Error('username and state are required');
  await kvSet(`${PENDING_PREFIX}${state}`, {
    username, codeVerifier, provider, tier, returnTo: returnTo === 'login' ? 'login' : 'forge', created_at: Date.now(),
...
export async function saveConnection(username, { provider, key, tier, models, funded = false, keyHash = null }) {
  ...
  await kvSet(`${KEY_PREFIX}${username}`, {
    ...
    // `funded` distinguishes a key WE minted and pay for from one the customer
    // brought. It decides whether a build is metered, so it is not cosmetic:
    // if a funded key were ever mistaken for the customer's own, every build
    // on it would skip metering and spend our balance unbounded. Defaults to
    // false so anything written by an older code path is treated as the
    // customer's own and metered conservatively — the safe direction to be
    // wrong in is charging ourselves less, never more.
    funded: Boolean(funded),
    // OpenRouter's identifier for a minted key, kept so it can be revoked
    // later. Not a secret: it cannot be used to make requests.
    key_hash: keyHash,
...
export async function getConnection(username) {
  ...
  return { ...safe, connected: Boolean(secret), funded: Boolean(record.funded) };
}

/**
 * Whether this account is running on a Brain WE fund. Separate from
 * getConnection so the check reads unambiguously at call sites that decide
 * whether to meter a request.
 */
export async function isFundedConnection(username) {
  if (!username) return false;
  const record = await kvGet(`${KEY_PREFIX}${username}`);
  return Boolean(record?.funded);
}
