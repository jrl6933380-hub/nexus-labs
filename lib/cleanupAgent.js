...
import { isClaimCheckStillValid } from './claimMismatch.js';
...
export function analyzeBoardHygiene(board, now = Date.now()) {
  ...
  const flagged = {
    ...
    // A claim_check flag is computed once, when the task is completed, and
    // then stored on the task. If the result is later corrected, the stored
    // flag goes stale and this sweep would otherwise re-report it on every
    // pass, forever — exactly what task 1789189303587-as3ta2 did, whose flag
    // recorded matched_phrase "not yet wired" that appears nowhere in its
    // current result or note. Revalidating here (read-only, never mutating
    // the task) means a corrected result actually clears the warning.
    completion_mismatches: tasks.filter((task) => isClaimCheckStillValid(task)),
  };
