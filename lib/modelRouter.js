// Provider routing for Nex's Anthropic-compatible message loop.
// Claude stays primary. Vercel AI Gateway is an independent backup that can
// run OpenAI (and optional additional models) through the same Messages API
// shape, so Nex's tools and safety gates do not change when a provider fails.

const ANTHROPIC_ENDPOINT = 'https://api.anthropic.com/v1/messages';
const GATEWAY_ENDPOINT = 'https://ai-gateway.vercel.sh/v1/messages';

// Gateway tier mapping.
//
// These were previously all pinned to the same cheapest model because the
// Gateway account could not invoke anything better. That is no longer true —
// the account has credits — and the pinning had a real cost: any Anthropic
// hiccup silently dropped EVERY request, including code work and Story
// Studio generation, onto a nano-class model. The visible symptom was Nex
// suddenly getting vague and losing track of context mid-session.
//
// As of 2026-09-13, all three tiers are overridden via NEX_GATEWAY_*_MODEL
// env vars to run on Claude through Gateway itself — heavy on
// anthropic/claude-opus-5, standard on anthropic/claude-sonnet-5, cheap on
// anthropic/claude-haiku-4.5. (Heavy was briefly dropped to Sonnet 5 over a
// cost concern that turned out to actually be a caching bug — the Gateway
// request path wasn't applying applyCaching(), so every message re-billed
// the full system prompt and full tool list uncached regardless of which
// model was set. Fixed; Opus 5 restored for heavy once real cost is no
// longer being inflated by that leak.) The direct Anthropic API key isn't
// funded right now, but Gateway bills Claude usage on its own separate
// credits, so this keeps Nex on Claude end to end without needing that key
// funded. NEX_GATEWAY_FALLBACK_MODELS is set to anthropic/claude-sonnet-5 as
// a backstop for Opus 5's occasional false-positive security classifier
// blocks. The defaults below are what's used only if those env vars are
// ever removed — keep them as a reasonable non-Anthropic baseline, not as
// the active configuration.
//
// Deliberate choice for these defaults specifically: they are NOT Anthropic
// models. This map is only ever reached after the direct Anthropic call
// already failed, and a large share of those failures are upstream
// (Anthropic API degraded or rate-limiting) rather than local. Falling back
// to anthropic/* through Gateway by default would route straight back into
// the provider that just failed. A genuine backup has to be a different
// house — hence Gemini/GPT as the fallback baseline, with Claude-via-
// Gateway currently forced on top via the env overrides above.
//
// Every entry stays overridable via NEX_GATEWAY_<TIER>_MODEL so a model can
// be swapped without a deploy if a name changes or a provider degrades.
export const DEFAULT_GATEWAY_MODELS = {
  // Trivial, high-volume work: classification, short summaries, routing
  // decisions. Still a real model, just not an expensive one.
  cheap: 'google/gemini-2.5-flash',
  // Normal conversational and reasoning work.
  standard: 'openai/gpt-5.6-sol',
  // Code, architecture, and anything where being wrong is expensive.
  heavy: 'openai/gpt-5.6-sol',
};

const DEFAULT_TIMEOUT_MS = 45_000;
const DEFAULT_DELEGATE_TIMEOUT_MS = 45_000;

export class AllProvidersUnavailableError extends Error {
  constructor(attempts) {
    super('All configured reasoning providers are temporarily unavailable.');
    this.name = 'AllProvidersUnavailableError';
    this.attempts = attempts;
  }
}

function timeoutMs(env) {
  const requested = Number(env.NEX_PROVIDER_TIMEOUT_MS);
  if (!Number.isFinite(requested)) return DEFAULT_TIMEOUT_MS;
  return Math.max(5_000, Math.min(requested, 90_000));
}

function gatewayModel(tier, env) {
  const key = `NEX_GATEWAY_${String(tier || 'standard').toUpperCase()}_MODEL`;
  return env[key] || DEFAULT_GATEWAY_MODELS[tier] || DEFAULT_GATEWAY_MODELS.standard;
}

function gatewayFallbackModels(env) {
  return String(env.NEX_GATEWAY_FALLBACK_MODELS || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);
}

function outputEffort(env) {
  return env.NEX_OUTPUT_EFFORT || 'medium';
}

// Anthropic only allows disabling thinking at effort high or below --
// xhigh/max require reasoning to stay on and return a 400 otherwise.
// The UI only ever offers low/medium/high (see nex-chat-bar.js and
// mission-control.html), so this guard is a defensive backstop, not
// the primary safeguard.
const EFFORT_LEVELS_ALLOWING_DISABLED_THINKING = new Set(['low', 'medium', 'high']);

function buildOutputConfig({ effortOverride, env }) {
  return { effort: effortOverride || outputEffort(env) };
}

function buildThinkingConfig({ thinkingEnabled, effort }) {
  if (thinkingEnabled === false && EFFORT_LEVELS_ALLOWING_DISABLED_THINKING.has(effort)) {
    return { type: 'disabled' };
  }
  return undefined; // omit entirely -- adaptive thinking stays on by default
}

// One same-provider bounce-back attempt for genuinely transient failures.
// Permanent request/auth failures and rate limits fail over immediately;
// aborted requests never restart. Non-streaming retries also share the
// original timeout budget, so this cannot turn one 45-second provider window
// into two and block an otherwise automatic customer build for 90 seconds.
const RETRY_DELAY_MS = 600;
const RETRYABLE_HTTP_STATUSES = new Set([408, 425]);
const RETRYABLE_NETWORK_CODES = new Set([
  'EAI_AGAIN', 'ECONNRESET', 'EPIPE', 'ETIMEDOUT',
  'UND_ERR_CONNECT_TIMEOUT', 'UND_ERR_SOCKET',
]);

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function retryDelayMs(env) {
  const requested = Number(env.NEX_PROVIDER_RETRY_DELAY_MS);
  if (!Number.isFinite(requested)) return RETRY_DELAY_MS;
  return Math.max(0, Math.min(requested, 5_000));
}

function isAbortError(error) {
  return error?.name === 'AbortError' || error?.code === 'ABORT_ERR';
}

function isTransientProviderError(error) {
  if (!error || isAbortError(error)) return false;
  const status = Number(error.status);
  if (Number.isFinite(status)) {
    return RETRYABLE_HTTP_STATUSES.has(status) || status >= 500;
  }
  const code = error?.cause?.code || error?.code;
  return error instanceof TypeError || RETRYABLE_NETWORK_CODES.has(code);
}

function providerHttpError(provider, status) {
  const error = new Error(`${provider} returned HTTP ${status}`);
  error.status = status;
  return error;
}

async function withOneRetry(fn, { timeoutMs = null, signal = null, delayMs = RETRY_DELAY_MS } = {}) {
  const startedAt = Date.now();
  try {
    return await fn(timeoutMs);
  } catch (firstError) {
    if (!isTransientProviderError(firstError) || signal?.aborted) throw firstError;

    const elapsedMs = Date.now() - startedAt;
    const remainingMs = timeoutMs == null ? null : timeoutMs - elapsedMs;
    if (remainingMs != null && remainingMs <= delayMs) throw firstError;

    await sleep(delayMs);
    if (signal?.aborted) throw firstError;

    const retryTimeoutMs = remainingMs == null ? null : remainingMs - delayMs;
    try {
      return await fn(retryTimeoutMs);
    } catch (secondError) {
      secondError.bounceBackAttempted = true;
      throw secondError;
    }
  }
}

async function postMessages({ provider, endpoint, headers, body, fetchFn, requestTimeoutMs }) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), requestTimeoutMs);

  try {
    const response = await fetchFn(endpoint, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
      signal: controller.signal,
    });
    const contentType = response.headers.get('content-type') || '';

    if (!response.ok || contentType.includes('text/html')) {
      // Read the response so provider diagnostics reach Vercel logs, but keep
      // the public error free of request bodies, prompts, and credentials.
      const responseText = await response.text();
      console.error(
        `${provider} rejected Nex request:`,
        response.status,
        responseText.slice(0, 500)
      );
      throw providerHttpError(provider, response.status);
    }

    return await response.json();
  } finally {
    clearTimeout(timer);
  }
}

async function openMessageStream({ provider, endpoint, headers, body, fetchFn, signal }) {
  const response = await fetchFn(endpoint, {
    method: 'POST',
    headers,
    body: JSON.stringify({ ...body, stream: true }),
    signal,
  });
  const contentType = response.headers.get('content-type') || '';

  if (!response.ok || contentType.includes('text/html') || !response.body) {
    const responseText = await response.text().catch(() => '');
    console.error(
      `${provider} rejected streaming request:`,
      response.status,
      responseText.slice(0, 500)
    );
    throw providerHttpError(provider, response.status);
  }

  return response;
}

/**
 * Send one Anthropic Messages-compatible request.
 *
 * The direct Anthropic API is attempted first unless NEX_FORCE_GATEWAY=true.
 * AI Gateway is then attempted with a tier-matched model. Tests inject
 * env/fetchFn; production uses process.env and the platform fetch.
 *
 * Returns { data, provider, model, degraded } — `degraded` is true when the
 * answer came from the backup path rather than the primary. Callers that
 * surface responses to a human should pass that through: silently serving a
 * fallback answer as if nothing happened is how a provider outage turns into
 * "why is Nex suddenly dumb?" with no visible cause.
 */
function withCacheBreakpoint(list) {
  if (!Array.isArray(list) || !list.length) return list;
  const marked = list.slice(0, -1);
  marked.push({ ...list[list.length - 1], cache_control: { type: 'ephemeral' } });
  return marked;
}

function messageContentAsBlocks(content) {
  return typeof content === 'string' ? [{ type: 'text', text: content }] : content;
}

function withMessageCacheBreakpoint(message) {
  return { ...message, content: withCacheBreakpoint(messageContentAsBlocks(message.content)) };
}

// Anthropic allows at most 4 cache_control breakpoints per request. System
// and tools already spend one each (2 total), leaving 2 for the message
// history. Placing them on the two most recent messages — a small sliding
// window at the tail that moves forward every call — means each new turn
// only pays the write premium on genuinely new content while still hitting
// cache on the turn immediately before it. This matters most for Nex's own
// tool-calling loop (lib/nexBrain.js), which can resend a rapidly growing
// messages array many times within a single user turn as tools run one
// after another; before this, that entire growing history was reprocessed
// uncached on every single iteration.
function withHistoryCacheBreakpoints(messages) {
  if (!Array.isArray(messages) || messages.length < 2) return messages;
  const cut = Math.max(0, messages.length - 2);
  return [
    ...messages.slice(0, cut),
    ...messages.slice(cut).map(withMessageCacheBreakpoint),
  ];
}

// Anthropic prompt caching: a cache_control breakpoint tells Anthropic
// "everything up through here is stable, reuse it instead of
// reprocessing." Nex's system prompt runs thousands of words (identity,
// board coordination, billing-action rules, hyperfocus, scoped-approval
// policy) and TOOLS carries 30+ full schemas — both are identical on
// nearly every call, so both get a breakpoint. Below Anthropic's
// minimum cacheable size this is silently a no-op, never an error, so
// it's safe to always apply.
function applyCaching(body) {
  const cachedSystem = typeof body.system === 'string'
    ? [{ type: 'text', text: body.system, cache_control: { type: 'ephemeral' } }]
    : body.system;
  return {
    ...body,
    system: cachedSystem,
    tools: withCacheBreakpoint(body.tools || []),
    messages: withHistoryCacheBreakpoints(body.messages),
  };
}

export async function routeMessage({
  tier = 'standard',
  claudeModel,
  body,
  env = process.env,
  fetchFn = fetch,
  // Tool defs that only make sense on a direct Anthropic call (e.g.
  // Anthropic's native web_search/web_fetch server tools) — merged
  // into the Anthropic request's tools array, left out of the Gateway
  // request entirely, since a Gateway model wouldn't understand them.
  anthropicServerTools = [],
  // Per-request overrides from the chat UI's effort/deep-thought
  // controls (nex-chat-bar.js, mission-control.html). Both are
  // optional; omitted means "use the server default" (NEX_OUTPUT_EFFORT,
  // thinking left on).
  effortOverride,
  thinkingEnabled = true,
}) {
  const attempts = [];
  const forceGateway = env.NEX_FORCE_GATEWAY === 'true';
  const requestTimeoutMs = timeoutMs(env);
  const outputConfig = buildOutputConfig({ effortOverride, env });
  const thinking = buildThinkingConfig({ thinkingEnabled, effort: outputConfig.effort });

  if (env.ANTHROPIC_API_KEY && !forceGateway) {
    try {
      const serverToolNames = new Set(anthropicServerTools.map((tool) => tool?.name).filter(Boolean));
      const directTools = [
        ...(body.tools || []).filter((tool) => !serverToolNames.has(tool?.name)),
        ...anthropicServerTools,
      ];
      const data = await withOneRetry((attemptTimeoutMs) => postMessages({
        provider: 'anthropic',
        endpoint: ANTHROPIC_ENDPOINT,
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': env.ANTHROPIC_API_KEY,
          'anthropic-version': '2023-06-01',
          // Required for the web_fetch server tool specifically; harmless
          // to send even on requests that only use web_search or neither.
          'anthropic-beta': 'web-fetch-2025-09-10',
        },
        body: applyCaching({
          ...body,
          model: claudeModel,
          // Prefer Anthropic's native implementation when a Gateway-compatible
          // local schema has the same name. Duplicate tool names are invalid.
          tools: directTools,
          output_config: outputConfig,
          ...(thinking ? { thinking } : {}),
        }),
        fetchFn,
        requestTimeoutMs: attemptTimeoutMs,
      }), { timeoutMs: requestTimeoutMs, delayMs: retryDelayMs(env) });
      return { data, provider: 'anthropic', model: data.model || claudeModel, degraded: false };
    } catch (error) {
      attempts.push({ provider: 'anthropic', error: error.message });
      console.error('Nex primary provider failed; trying AI Gateway:', error.message);
    }
  } else {
    attempts.push({
      provider: 'anthropic',
      error: forceGateway ? 'bypassed by NEX_FORCE_GATEWAY' : 'not configured',
    });
  }

  if (env.AI_GATEWAY_API_KEY) {
    const model = gatewayModel(tier, env);
    const fallbackModels = gatewayFallbackModels(env);
    const providerOptions = fallbackModels.length
      ? { gateway: { models: fallbackModels } }
      : undefined;

    try {
      const data = await withOneRetry((attemptTimeoutMs) => postMessages({
        provider: 'vercel-ai-gateway',
        endpoint: GATEWAY_ENDPOINT,
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': env.AI_GATEWAY_API_KEY,
          'anthropic-version': '2023-06-01',
        },
        body: {
          ...applyCaching(body),
          model,
          output_config: outputConfig,
          ...(thinking ? { thinking } : {}),
          ...(providerOptions ? { providerOptions } : {}),
        },
        fetchFn,
        requestTimeoutMs: attemptTimeoutMs,
      }), { timeoutMs: requestTimeoutMs, delayMs: retryDelayMs(env) });
      return {
        data,
        provider: 'vercel-ai-gateway',
        model: data.model || model,
        degraded: true,
      };
    } catch (error) {
      attempts.push({ provider: 'vercel-ai-gateway', error: error.message });
      console.error('Nex backup provider failed:', error.message);
    }
  } else {
    attempts.push({ provider: 'vercel-ai-gateway', error: 'not configured' });
  }

  throw new AllProvidersUnavailableError(attempts);
}

/**
 * Open a streaming Anthropic-Messages-compatible response without consuming
 * the body. The caller owns the stream and the deadline through `signal`.
 *
 * `gatewayOnly` is used by customer-facing generation routes. Those routes
 * must bill and fail over in the centrally funded AI Gateway account instead
 * of depending on a separate direct-Anthropic credit balance.
 */
export async function routeMessageStream({
  tier = 'standard',
  claudeModel,
  body,
  env = process.env,
  fetchFn = fetch,
  signal,
  gatewayOnly = false,
}) {
  const attempts = [];

  if (!gatewayOnly && env.ANTHROPIC_API_KEY && env.NEX_FORCE_GATEWAY !== 'true') {
    try {
      const response = await withOneRetry(() => openMessageStream({
        provider: 'anthropic',
        endpoint: ANTHROPIC_ENDPOINT,
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': env.ANTHROPIC_API_KEY,
          'anthropic-version': '2023-06-01',
        },
        body: applyCaching({ ...body, model: claudeModel }),
        fetchFn,
        signal,
      }), { signal, delayMs: retryDelayMs(env) });
      return { response, provider: 'anthropic', model: claudeModel, degraded: false };
    } catch (error) {
      attempts.push({ provider: 'anthropic', error: error.message });
      console.error('Streaming primary provider failed; trying AI Gateway:', error.message);
    }
  } else {
    attempts.push({
      provider: 'anthropic',
      error: gatewayOnly || env.NEX_FORCE_GATEWAY === 'true' ? 'bypassed for gateway-only route' : 'not configured',
    });
  }

  if (env.AI_GATEWAY_API_KEY) {
    const model = gatewayModel(tier, env);
    const fallbackModels = gatewayFallbackModels(env);
    const providerOptions = fallbackModels.length
      ? { gateway: { models: fallbackModels } }
      : undefined;
    try {
      const response = await withOneRetry(() => openMessageStream({
        provider: 'vercel-ai-gateway',
        endpoint: GATEWAY_ENDPOINT,
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': env.AI_GATEWAY_API_KEY,
          'anthropic-version': '2023-06-01',
        },
        body: {
          ...applyCaching(body),
          model,
          ...(providerOptions ? { providerOptions } : {}),
        },
        fetchFn,
        signal,
      }), { signal, delayMs: retryDelayMs(env) });
      return { response, provider: 'vercel-ai-gateway', model, degraded: false };
    } catch (error) {
      attempts.push({ provider: 'vercel-ai-gateway', error: error.message });
      console.error('Streaming AI Gateway request failed:', error.message);
    }
  } else {
    attempts.push({ provider: 'vercel-ai-gateway', error: 'not configured' });
  }

  throw new AllProvidersUnavailableError(attempts);
}

/**
 * Send one Anthropic Messages-compatible request to a SPECIFIC named model
 * through AI Gateway — no tier, no automatic fallback chain, no attempt at
 * Anthropic first. This is for deliberate delegation ("have Llama look at
 * this") rather than reliability routing, so it fails loudly instead of
 * silently trying something else when the named model is unavailable.
 *
 * Reuses the exact same Gateway endpoint and Anthropic-compatible request
 * shape as routeMessage's fallback path — this is not a new provider
 * integration, just an explicit model string instead of a tier lookup.
 * Gateway accepts "creator/model-name" strings, e.g.:
 *   "google/gemini-2.5-flash", "meta/llama-3.3-70b-instruct",
 *   "openai/gpt-5.6-sol", "anthropic/claude-sonnet-4.5".
 *
 * NOTE ON COST: this does not mean the call is free. Whether it is depends
 * entirely on account-level configuration (BYOK in the Vercel AI Gateway
 * dashboard against a provider key that itself has a free tier) that this
 * function has no way to see or control. It only ever reflects the real
 * result Gateway returns.
 */
export async function routeToModel({
  model,
  body,
  env = process.env,
  fetchFn = fetch,
}) {
  if (!model || !String(model).trim()) {
    throw new Error('model is required, e.g. "google/gemini-2.5-flash" or "meta/llama-3.3-70b-instruct"');
  }
  if (!env.AI_GATEWAY_API_KEY) {
    throw new Error('AI_GATEWAY_API_KEY is not configured — delegating to a specific model requires the Vercel AI Gateway key.');
  }

  const data = await postMessages({
    provider: `vercel-ai-gateway:${model}`,
    endpoint: GATEWAY_ENDPOINT,
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': env.AI_GATEWAY_API_KEY,
      'anthropic-version': '2023-06-01',
    },
    body: { ...body, model },
    fetchFn,
    requestTimeoutMs: env.NEX_DELEGATE_TIMEOUT_MS
      ? Math.max(5_000, Math.min(Number(env.NEX_DELEGATE_TIMEOUT_MS) || DEFAULT_DELEGATE_TIMEOUT_MS, 90_000))
      : DEFAULT_DELEGATE_TIMEOUT_MS,
  });

  return { data, provider: 'vercel-ai-gateway', model: data.model || model };
}
