...
// One same-request bounce-back attempt before a provider is counted as
// failed. A lot of "AllProvidersUnavailableError" hits are a single
// transient blip (a dropped connection, a momentary 5xx) rather than a real
// outage -- retrying once with a short delay clears those silently, so the
// human never sees the safe-fallback message for something that would have
// worked a second later. This does not chase a genuine outage: it is one
// extra attempt per provider, not a loop, and every attempt (including the
// retry) still lands in `attempts` for diagnostics if everything fails.
...
export async function routeMessage({
  ...
  if (env.ANTHROPIC_API_KEY && !forceGateway) {
    try {
      const data = await withOneRetry(() => postMessages({
        provider: 'anthropic',
