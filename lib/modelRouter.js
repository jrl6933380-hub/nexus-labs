...
// env vars to run on Claude through Gateway itself (anthropic/claude-opus-5,
// anthropic/claude-sonnet-5, anthropic/claude-haiku-4.5) — the direct
// Anthropic API key isn't funded right now, but Gateway bills Claude usage
// on its own separate credits, so this keeps Nex on Claude end to end
// without needing that key funded. NEX_GATEWAY_FALLBACK_MODELS is set to
// anthropic/claude-sonnet-5 as a backstop for Opus 5's occasional false-
// positive security classifier blocks. The defaults below are what's used
// only if those env vars are ever removed — keep them as a reasonable
// non-Anthropic baseline, not as the active configuration.
//
