...
// env vars to run on Claude through Gateway itself — heavy and standard both
// on anthropic/claude-sonnet-5 (Opus 5 was tried for heavy but is priced for
// long-horizon agentic work well beyond what this project currently needs;
// Sonnet 5 gets near-Opus results on coding/agentic tasks at a fraction of
// the cost), cheap on anthropic/claude-haiku-4.5. The direct Anthropic API
// key isn't funded right now, but Gateway bills Claude usage on its own
// separate credits, so this keeps Nex on Claude end to end without needing
// that key funded. NEX_GATEWAY_FALLBACK_MODELS is set to
// anthropic/claude-haiku-4.5 as a general backstop. The defaults below are
// what's used only if those env vars are ever removed — keep them as a
// reasonable non-Anthropic baseline, not as the active configuration.
//
