...
const DEFAULT_CONFIG = Object.freeze({
  ...
  // Free allowance: ONE full build, FOUR edits, and a handful of chat
  // turns, at 15 + (4 x 2) + 7 = 30 credits.
  //
  // The shape matters more than the total. It is deliberately enough to
  // reach a page that WORKS, and deliberately not enough to polish it
  // to finished. Someone who runs out mid-repair is left holding a
  // broken page and has been given a reason to leave; someone who runs
  // out holding a working page they want to improve has been given a
  // reason to pay. Widening this to ~10 edits removes that entirely,
  // because most small pages are genuinely finishable in ten.
  //
  // At Haiku rates this is roughly $0.06 of inference per account per
  // day, so 50 accounts all maxing out every single day costs about
  // $90/month — a number that can be absorbed while conversion is
  // still unknown.
  dailyCreditsLimit: 30,
  ...
  //
  // 5,000 credits is ~$10/day of inference. At a 30-credit daily
  // allowance that is roughly 165 accounts exhausting themselves on the
  // same day, so it will not fire in normal operation — which is the
  // point. It is the brake for when something is wrong, not a limit
  // anyone should meet while using the product as intended.
  globalDailyCreditCeiling: 5000,
});
