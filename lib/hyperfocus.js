...
const LEASE_MS = 10 * 60 * 1000;                 // exclusive lease, opt-in only
...
// CONCURRENCY MODEL — worth reading before changing:
//
// The default is OPTIMISTIC, guarded by the version counter (if_version),
// NOT by an exclusive lease. An earlier draft grabbed a 10-minute lease
// automatically on every write, and that quietly broke the entire
// feature: the whole point is a handoff, so the moment ChatGPT published
// its context, Claude — the agent being handed the work — was locked out
// for ten minutes. The lock was fighting the use case.
//
// So: a write does not take a lease unless the caller explicitly asks for
// one with `hold: true` ("I'm actively iterating, don't let anyone else
// interleave"). Writing WITHOUT `hold` releases any lease the caller was
// holding, because publishing your context is the act of handing off.
// Split-brain edits are still prevented — by if_version, which rejects a
// write built on stale content regardless of who holds what.
function applyLease(workspace, agent, hold) {
  if (hold) {
    workspace.manifest.lease = { holder: agent, acquired_at: now(), expires_at: now() + LEASE_MS };
    return;
  }
  if (leaseHolder(workspace.manifest) === agent) {
    workspace.manifest.lease = null;
  }
}
 ...
 *
 * `hold` defaults to false because publishing is normally the handoff
 * itself; see the concurrency note above applyLease().
 */
export async function publishChatContext({
  ...
  applyLease(workspace, actor, hold);
