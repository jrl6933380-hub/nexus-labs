  ...
  return {
    async queue({ userId, projectId, request, reason, currentHtml = '' }) {
      ...
      return {
        ...
        message: `This one needs more than the instant builder, so I opened Build Team ticket ${id}. A builder is on it now, and it comes to me for review before it goes out.`,
      };
    },
    ...
    // Permanently drop an escalation record. Needed because board tasks and
    // the underlying Redis record are separate: deleting the tasks left the
    // record behind, so list()/get() kept resurfacing dead escalations as if
    // they were live work waiting to be built.
    async remove(id) {
      if (!/^forge-[a-zA-Z0-9_-]+$/.test(String(id || ''))) throw new Error('Invalid Forge escalation id');
      const existing = await store.get(id);
      if (!existing) throw new Error('Forge escalation not found');
      await store.remove(id);
      return { id, deleted: true, wasStatus: existing.status || null };
    },
  };
}
...
export const deleteForgeEscalation = (id) => roomEscalator.remove(id);
