...
function formatMemory(memory) {
  ...
  const provenance = memory.provenance ? ` provenance=${memory.provenance}` : '';
  const confidence = Number.isFinite(Number(memory.confidence)) ? ` confidence=${Number(memory.confidence).toFixed(2)}` : '';
  const project = memory.project ? ` project=${compactText(memory.project).replace(/\s+/gu, '_')}` : '';
  return `- [id=${memory.id || 'unknown'} category=${memory.category || 'fact'}${scope}${provenance}${confidence}${project}${tags}${timestamp ? ` timestamp=${timestamp}` : ''}] ${compactText(memory.content)}`;
}
...
export function compileNexContext({
  ...
  const budgets = lane === 'code'
    ? { memory: 5500, live: 7000, snapshot: 8500, skills: 4000 }
    : { memory: 7500, live: 8000, snapshot: 6000, skills: 3500 };
  ...
  const sections = [
    ...
    section({
      id: 'Reviewed Nex skills',
      provenance: 'repository:nex-skills',
      trust: 'reviewed-guidance',
      freshness: 'deployed-revision',
      content: skillsText,
