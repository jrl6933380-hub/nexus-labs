// /lib/boardDispatcher.js
// Wires the dispatcher (task 02) into the shared board (lib/board.js),
// mirroring atomic dispatch outcomes into board task status so humans
// and other agents watching the board see real state.
...
// in fireClaudeRoutine for 'claude-routine' here, and task 04 (ChatGPT
// wake relay) will add its own entry the same way, without this file
// needing to know anything provider-specific. An agent with no entry
// in `wake` (e.g. Nex, permanent/local workers) is claimed exactly as
// before — nothing to wake, nothing changes.
