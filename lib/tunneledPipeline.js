...
export function createTunneledPipelineService({ board, store, delegate = routeToModel, env = process.env }) {
  ...
  const LANE_WORK_CONTRACT = [
    ...
    'If the goal or acceptance criteria mention a Forge escalation id (forge-...), call get_forge_escalation with that id FIRST. That is where the real customer request and existing site HTML live — it is not repo source, so searching the repo for it will always fail.',
