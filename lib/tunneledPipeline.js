  ...
  return {
    ...
    async submitReview({ pipeline_id, decision, summary, evidence = [], corrections = [] }) {
      ...
      if (decision === 'needs_changes') {
        ...
        await board.updateProgress({ id: run.parent_task_id, status: 'building', note: `Reviewer requested corrections: ${reviewResult.corrections.join(' | ') || reviewResult.summary}` });
        return save(run);
      }
