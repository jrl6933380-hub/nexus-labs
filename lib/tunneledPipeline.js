...
// `github` is injectable so tests can run the real lane loop hermetically;
// it defaults to the live GitHub module in production.
export function createTunneledPipelineService({ board, store, delegate = routeToModel, env = process.env, github = gh }) {
    ...
    return async function execute(name, input) {
      switch (name) {
        ...
        case 'search_repo_code':
          return github.searchCode({ owner, repo, query: input.query });
        case 'read_repo_file':
          return github.readFile({ owner, repo, path: input.path, branch: branchName, start_line: input.start_line, end_line: input.end_line });
        case 'patch_repo_file':
          return github.patchFile({ owner, repo, path: input.path, branch: branchName, replacements: input.replacements });
        case 'create_repo_file':
          return github.createOrUpdateFile({ owner, repo, path: input.path, branch: branchName, content: input.content });
        case 'inspect_branch_diff':
          return github.inspectBranchDiff({ owner, repo, head: branchName, base: baseBranch });
        case 'create_pull_request':
          return github.createPullRequest({ owner, repo, title: input.title, body: input.body, head: branchName, base: baseBranch });
        default:
  ...
  async function runLaneDelegate({ pipelineId, lane, description, laneTaskId, owner, repo, baseBranch }) {
    ...
    // Required evidence is deliberately branch + write, NOT source_read.
    // source_read only fires on read_repo_file/search_repo_code, and a
    // greenfield build (new customer site, nothing in the repo yet) has
    // nothing to read — requiring it meant a from-scratch build could never
    // pass the gate no matter how much real work it did. Reading before
    // MODIFYING an existing file is still required by LANE_WORK_CONTRACT, and
    // createOrUpdateFile/patchFile independently guard against blind
    // clobbering via assertSafeFileReplacement.
    const tracker = createEvidenceTracker({ lane, requireEvidence: ['non_live_branch', 'write_succeeded'] });

    try {
      await github.createBranch({ owner, repo, branch: branchName, from_branch: baseBranch });
