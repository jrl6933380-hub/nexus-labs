...
function laneDescription({ goal, lane, acceptanceCriteria, pipelineId }) {
  const focus = lane === 'layout'
    ...
    : 'Own the whole request end-to-end: structure, behavior, data flow, and visual polish. No second lane is coming for this job -- ship a complete, working result yourself.';
  return [
    `Pipeline: ${pipelineId}`,
    `Goal: ${goal}`,
    focus,
...
export function createTunneledPipelineService({ board, store, delegate = routeToModel, env = process.env }) {
  ...
  async function start({ goal, owner, repo, branch, acceptance_criteria = [], split = false, build_agent = 'nex', layout_agent = 'claude', design_agent = 'chatgpt', reviewer_agent = 'nex' }) {
    ...
    // Default is ONE build lane, one worker -- not an automatic layout+design
    // fork. Every lane is a full independent model tool-loop (see
    // runLaneDelegate), so forking on every escalation regardless of size was
    // paying double for jobs that never needed a second set of hands, and
    // meant a shared bug (like the max_tokens truncation) got hit twice in
    // parallel instead of once. split:true opts into the real second lane for
    // work that is genuinely separable; it stays the exception, not the rule.
    if (!split) {
      const parent = await board.createTask({
        title: `Pipeline: ${clean(goal, 120)}`,
        owner: reviewer_agent,
        description: `Parent coordinator for ${id}. It cannot complete until the build lane and reviewer QA pass.`,
      });
      const base = { id, goal: clean(goal, 900), owner, repo, branch: branch || null, acceptance_criteria: criteria, split: false,
      ...
      const buildTask = await board.createTask({ title: `Build lane: ${clean(goal, 100)}`, owner: build_agent, description: buildDescription });
      base.lane_task_ids = { build: buildTask.id };
      await board.updateProgress({ id: parent.id, status: 'building', note: 'Single build lane created; delegating to one model, reviewer opens once it submits evidence.' });
      await save(base);
      await runLaneDelegate({ pipelineId: id, lane: 'build', description: buildDescription, laneTaskId: buildTask.id, owner, repo, baseBranch });
      return load(id);
    }
    ...
    const base = { id, goal: clean(goal, 900), owner, repo, branch: branch || null, acceptance_criteria: criteria, split: true,
      status: 'lanes_running', parent_task_id: parent.id, reviewer_agent, review_task_id: null, final_handoff: null, created_at: Date.now(), updated_at: Date.now(), lanes: {} };
  ...
  async function submitLaneResult({ pipeline_id, lane, summary, evidence = [] }) {
    ...
    const allLanesDone = run.split ? (run.lanes.layout && run.lanes.design) : Boolean(run.lanes.build);
    if (allLanesDone) {
      ...
      const note = run.split
        ? `Both lanes returned evidence; reviewer gate ${review.id} is now required.`
        : `Build lane returned evidence; reviewer gate ${review.id} is now required.`;
      await board.updateProgress({ id: run.parent_task_id, status: 'testing', note });
    }
    return save(run);
  }
  ...
  async function submitReview({ pipeline_id, decision, summary, evidence = [], corrections = [] }) {
    ...
    if (decision === 'needs_changes') {
      ...
      const laneTaskIds = run.split ? [run.lane_task_ids.layout, run.lane_task_ids.design] : [run.lane_task_ids.build];
      await Promise.all([
        ...laneTaskIds.map((laneTaskId) => board.updateProgress({ id: laneTaskId, status: 'building', note: correctionNote })),
        board.updateProgress({ id: run.parent_task_id, status: 'building', note: `${correctionNote} Reopened for revision.` }),
      ]);
      return save(run);
    }
