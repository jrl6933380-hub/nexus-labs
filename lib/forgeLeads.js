...
export async function createLead(input, { createdBy } = {}) {
  ...
  const lead = {
    ...
    history: [],
    siteHtml: null,
    siteStatus: 'none', // 'none' | 'generating' | 'ready' | 'error'
    siteGeneratedAt: null,
    // Filled in by lib/forgePlaces.js after creation (best-effort, see
    // api/forge-leads.js) — null until enrichment succeeds.
    rating: null,
    reviewCount: null,
    placesWebsite: null,
    createdAt: now,
...
// Saves the generated/edited demo site for a lead. Kept as its own
// small write (not folded into recordDisposition) since site
// generation and call outcomes happen on independent timelines — a
// site can be regenerated mid-call without touching disposition state.
export async function updateLeadSite(id, { html, status }) {
  const lead = await getLead(id);
  if (!lead) throw new Error('No such lead.');
  if (html !== undefined) lead.siteHtml = html;
  if (status !== undefined) lead.siteStatus = status;
  if (status === 'ready') lead.siteGeneratedAt = Date.now();
...
// Best-effort enrichment write from lib/forgePlaces.js. Never throws
// on missing data — callers pass whatever fetchBusinessSignals()
// returned, including nulls, and this just records it.
export async function updateLeadEnrichment(id, { rating, reviewCount, placesWebsite } = {}) {
  const lead = await getLead(id);
  if (!lead) throw new Error('No such lead.');
  if (rating !== undefined) lead.rating = rating;
  if (reviewCount !== undefined) lead.reviewCount = reviewCount;
  if (placesWebsite !== undefined) lead.placesWebsite = placesWebsite;
  lead.updatedAt = Date.now();
  await redisCommand(['HSET', LEADS_KEY, id, JSON.stringify(lead)]);
  return lead;
}
