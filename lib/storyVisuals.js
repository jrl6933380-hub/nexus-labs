...
function bubbleWidthRange(line) {
  ...
  if (length <= 32) return { min:24, max:30 };
  if (length <= 70) return { min:28, max:38 };
  return { min:34, max:46 };
}
...
- Use width 24-30 for short lines, 28-38 for medium lines, and 34-46 for long lines. If two valid placements are possible, choose the one with fewer text lines and more breathing room.`;
}
