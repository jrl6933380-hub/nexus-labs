...
THIS SET AND CAMERA
Visual role: ${cleanText(panel?.visualRole, 40) || 'story beat'}
...
const ARTWORK_ISSUES = new Set(['generated_text', 'blank_lettering_box', 'no_safe_lettering_space', 'unauthorized_character']);

export function buildBackgroundPlateVisionPrompt({ panel }) {
  return `Inspect this proposed comic SET PLATE before any actors are composited. You are the set-continuity worker in Nex's art department.
...
This image must be a clean empty set: scenery, atmosphere, architecture, furniture, weather, and non-held environmental objects only. The live-stage renderer adds every character later as a separate transparent performance layer.

Return ONLY JSON:
{"verdict":"clean|regenerate","issues":["unauthorized_character|generated_text|blank_lettering_box|no_safe_lettering_space"]}

- Return "regenerate" if you can see any person, character, creature, face, body, human-shaped silhouette, crowd, humanoid statue, character reflection, held weapon, or foreground performer. Even one unauthorized person fails the set plate.
- Return "regenerate" if there are any words, letters, numerals, signage, captions, speech balloons, empty boxes, UI, or lettering placeholders.
- Return "regenerate" if the planned stage has no naturally quiet background area around the action zone.
- Return "clean" with an empty issues array only when this is a believable empty set ready for separate actors. Do not approve based on the prompt; inspect the actual pixels.`;
}

const COMPOSITE_ISSUES = new Set([
  'actor_too_large','actor_too_small','actor_off_canvas','actor_floating',
  'actor_wrong_depth','actor_wrong_pose','lighting_mismatch','set_has_character','generated_text',
]);

export function buildSceneCompositeReviewPrompt({ panel }) {
  const actors = (Array.isArray(panel?.scene?.actors) ? panel.scene.actors : [])
    .map((actor) => `${cleanText(actor.id,64)} = ${cleanText(actor.name,80)}`)
    .join('\n');
  return `You are Nex's composite supervisor. Inspect the actual finished comic-stage pixels after the empty set and transparent actor performances have been assembled.

STORY INTENT
Visual role: ${cleanText(panel?.visualRole,40) || 'action'}
Beat: ${cleanText(panel?.beat,700)}
Shot: ${cleanText(panel?.shot,200)}
Actors: ${actors || 'none'}

Return ONLY JSON:
{"verdict":"pass|adjust","issues":["actor_too_large|actor_too_small|actor_off_canvas|actor_floating|actor_wrong_depth|actor_wrong_pose|lighting_mismatch|set_has_character|generated_text"],"actors":[{"actorId":"known id","desiredBounds":{"x":number,"y":number,"width":number,"height":number},"confidence":number}]}

Coordinates are percentages of the complete panel. desiredBounds describes the actor's visible final body rectangle, not transparent PNG padding.
- Pass only when every actor belongs naturally in the room: believable scale for the camera depth, clear floor contact unless the beat explicitly requires flight, full intended silhouette on canvas, useful composition, and no accidental person baked into the set.
- Use adjust when position or size can fix the scene. Return every staged actor exactly once with a desired visible rectangle.
- Keep desired rectangles inside x=2..98 and y=2..96. Never make a non-close-up actor taller than 48% of the panel. Establishing/detail actors should usually be 18-30%; action/reveal 26-44%; reaction close-ups 38-60%; aftermath 22-38%.
- Do not center every actor. Use the room's floor, perspective, doors, furniture, action path, and negative space. Preserve intentional airborne action only when the written beat requires it.
- If the actor is already well composed, return its current visible rectangle. Inspect pixels, not prompt claims.`;
}
...
export function parseBackgroundPlateInspection(raw) {
  const parsed = jsonObject(raw);
  if (!parsed || !['clean', 'regenerate'].includes(parsed.verdict)) return null;
  const issues = Array.isArray(parsed.issues)
    ? [...new Set(parsed.issues.filter((issue) => ARTWORK_ISSUES.has(issue)))]
    : [];
  if (parsed.verdict === 'clean') return issues.length ? null : {verdict:'clean',issues:[]};
  return issues.length ? {verdict:'regenerate',issues} : null;
}

function roleHeightLimit(role) {
  return ({establishing:30,detail:30,action:44,reaction:60,reveal:46,aftermath:38})[role] || 46;
}

export function parseSceneCompositeReview(raw, panel) {
  const parsed = jsonObject(raw);
  if (!parsed || !['pass','adjust'].includes(parsed.verdict)) return null;
  const issues = Array.isArray(parsed.issues)
    ? [...new Set(parsed.issues.filter((issue) => COMPOSITE_ISSUES.has(issue)))]
    : [];
  if (parsed.verdict === 'pass' && issues.length) return null;
  const known = new Set((panel?.scene?.actors || []).map((actor) => cleanText(actor.id,64)).filter(Boolean));
  const entries = Array.isArray(parsed.actors) ? parsed.actors : [];
  if (entries.length !== known.size) return null;
  const seen = new Set();
  const heightLimit = roleHeightLimit(cleanText(panel?.visualRole,40));
  const actors = [];
  for (const entry of entries) {
    const actorId = cleanText(entry?.actorId,64);
    const box = entry?.desiredBounds || {};
    const values = [box.x,box.y,box.width,box.height,entry?.confidence].map(Number);
    if (!known.has(actorId) || seen.has(actorId) || !values.every(Number.isFinite)) return null;
    const [rawX,rawY,rawWidth,rawHeight,confidence] = values;
    if (confidence < .55 || rawWidth <= 2 || rawHeight <= 2) return null;
    const height = Math.min(heightLimit,Math.max(10,rawHeight));
    const width = Math.min(55,Math.max(6,rawWidth * (height / rawHeight)));
    const x = Math.max(2,Math.min(98 - width,rawX));
    const y = Math.max(2,Math.min(96 - height,rawY));
    seen.add(actorId);
    actors.push({
      actorId,
      desiredBounds:{x:Math.round(x*10)/10,y:Math.round(y*10)/10,width:Math.round(width*10)/10,height:Math.round(height*10)/10},
      confidence:Math.round(Math.min(1,confidence)*100)/100,
    });
  }
  return {verdict:parsed.verdict,issues,actors};
}
...
export async function inspectBackgroundPlate({
  panel,
  imageDataUrl,
  userId = '',
  env = process.env,
  ...
  if (!env.AI_GATEWAY_API_KEY) throw new Error('Story Studio set inspection is not configured');
  parseImageDataUrl(imageDataUrl);
  const controller = new AbortController();
  const timeoutMs = Math.max(8_000, Math.min(Number(env.STORY_STUDIO_VISION_TIMEOUT_MS) || 20_000, 30_000));
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    const response = await fetchFn(GATEWAY_IMAGE_ENDPOINT, {
      ...
      body:JSON.stringify({
        model,
        messages:[{role:'user',content:[
          {type:'text',text:buildBackgroundPlateVisionPrompt({panel})},
          {type:'image_url',image_url:{url:imageDataUrl}},
        ]}],
        max_tokens:300,
        temperature:0,
        providerOptions:{gateway:{
          ...(userId ? {user:String(userId).slice(0,120)} : {}),
          tags:['feature:story-studio-set-qa','role:set-continuity-worker'],
        }},
        stream:false,
      }),
      signal:controller.signal,
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(`Story Studio set inspector returned HTTP ${response.status}`);
    const inspection = parseBackgroundPlateInspection(gatewayText(data));
    if (!inspection) throw new Error('Story Studio set inspector returned unusable JSON');
    return inspection;
  } finally {
    clearTimeout(timer);
  }
...
export async function reviewSceneComposite({
  panel,
  previewDataUrl,
  userId = '',
  env = process.env,
  ...
  if (!env.AI_GATEWAY_API_KEY) throw new Error('Story Studio composite review is not configured');
  parseImageDataUrl(previewDataUrl);
  const controller = new AbortController();
  const timeoutMs = Math.max(8_000,Math.min(Number(env.STORY_STUDIO_VISION_TIMEOUT_MS) || 20_000,30_000));
  const timer = setTimeout(() => controller.abort(),timeoutMs);
  try {
    ...
    const response = await fetchFn(GATEWAY_IMAGE_ENDPOINT,{
      ...
      body:JSON.stringify({
        model,
        messages:[{role:'user',content:[
          {type:'text',text:buildSceneCompositeReviewPrompt({panel})},
          {type:'image_url',image_url:{url:previewDataUrl}},
        ]}],
        max_tokens:700,temperature:0,stream:false,
        providerOptions:{gateway:{
          ...(userId ? {user:String(userId).slice(0,120)} : {}),
          tags:['feature:story-studio-composite-qa','role:composite-supervisor'],
        }},
      }),
      signal:controller.signal,
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(`Story Studio composite supervisor returned HTTP ${response.status}`);
    const review = parseSceneCompositeReview(gatewayText(data),panel);
    if (!review) throw new Error('Story Studio composite supervisor returned unusable JSON');
    return review;
  } finally {
    clearTimeout(timer);
  }
...
export async function generateBackgroundPlate({
  ...
  const corrections = (Array.isArray(correctionIssues) ? correctionIssues : []).map((issue) => ({
    ...
    unauthorized_character:'The prior set plate contained a person or character. Remove every human, face, silhouette, crowd, creature, reflection, and foreground performer. Return scenery only; actors are added separately.',
    inspection_failed:'The prior set plate could not pass visual inspection. Return an unmistakably empty, text-free set with simple open staging space and no people, creatures, silhouettes, reflections, signs, or lettering.',
...
export async function generatePanelVisual({
  ...
  const correctionRules = (Array.isArray(correctionIssues) ? correctionIssues : []).map((issue) => ({
    ...
    unauthorized_character:'The prior attempt included an unplanned character. Keep only the named staged actors and remove all other people, silhouettes, and reflections.',
  }[issue])).filter(Boolean);
