...
{"artwork":"clean|regenerate","issues":["generated_text|blank_lettering_box|no_safe_lettering_space"],"placements":[{"index":0,"side":"left|right","x":number,"y":number,"width":number}]}
...
export function parsePanelVisualInspection(raw, dialogue) {
  ...
  if (parsed.artwork === 'regenerate') {
    ...
    return { artwork:'regenerate', issues, placements:[] };
  }
  if (issues.length) return null;
  ...
  return { artwork:'clean', issues:[], placements:placements || [] };
}
...
export async function inspectPanelVisual({
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      const inspection = parsePanelVisualInspection(gatewayText(data), dialogue);
...
export const __internals = { ARTWORK_ISSUES, GATEWAY_IMAGE_ENDPOINT, MAX_IMAGE_BYTES, PANEL_LIMIT, assetKey, bubbleHeight };
