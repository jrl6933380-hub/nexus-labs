...
const COUNCIL_SIGNAL = /\b(brain crew|crew mode|use (?:the )?crew|council|multiple models|multi[- ]model|second opinion|specialists?|team of models)\b/iu;
...
export function planCognitiveRun({ message, forcedTier = null, toolContext = {} } = {}) {
  ...
  // Crew Mode is a separate, explicit cost decision. Complexity, risk,
  // Deep Thought, and a heavy model selection may strengthen Nex's direct
  // reasoning, but must never fan out into scout/architect/reviewer calls.
  // Only an explicit operator request or forceCrew=true may activate it.
  // Approval gates for dangerous actions remain independent and always apply.
