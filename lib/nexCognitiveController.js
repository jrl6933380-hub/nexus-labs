import { contractFor, laneFor, readContractSkill } from './nexLanes.js';
...
export function planCognitiveRun({ message, forcedTier = null, toolContext = {} } = {}) {
  ...
  // TEMPORARY (2026-09-18): auto-crew-on-complexity disabled while funds are
  // tight. Crew mode (scout+architect+independent reviewer) now only runs
  // when explicitly requested (forceCrew, or saying "brain crew"/"second
  // opinion" etc.) instead of firing automatically on a high complexity/risk
  // score. This does NOT touch the separate hard approval gate for actually
  // dangerous actions (merge/deploy/delete/charge/credentials) -- that stays
  // enforced regardless of crew mode. To restore automatic crew-on-risk,
  // change the line below back to:
  //   crewRequested || (selected.lane === 'code' && score >= 4) ? 'crew' : 'direct'
  const mode = crewRequested ? 'crew' : 'direct';
...
export function resolveCognitiveTier(classifiedTier, forcedTier, plan) {
  const preferred = forcedTier && Object.hasOwn(TIER_WEIGHT, forcedTier) ? forcedTier : classifiedTier;
  const classified = Object.hasOwn(TIER_WEIGHT, preferred) ? preferred : 'standard';
...
// they are read here unconditionally via readContractSkill (never through
// selectNexSkills' keyword scoring) rather than through the scored skill
// path. The real source of truth is nex-skills/standing-policy/SKILL.md;
// editing that file changes what every turn loads, no code review of this
// module required. The inline fallback below only exists so a
// missing/corrupt skill file degrades to known-good text instead of
// silently dropping the policy -- it is a safety net, not the primary copy,
// and should stay identical to the skill file's instructions.
...
export const STANDING_POLICY = readContractSkill('standing-policy') || [
