...
function runRoutingTrials() {
  const trials = [
    ...
    planTrial('production-migration-uses-crew', 'Migrate the production auth database, then update tenant permissions and deploy it.', { lane: 'code', mode: 'crew', risk: 'high', minimumTier: 'heavy' }, { toolContext: { deepThoughtRequested: true } }),
  ...
  trials.push(result(
    ...
    'routing',
    resolveCognitiveTier('cheap', null, plan) === 'standard' && resolveCognitiveTier('cheap', 'cheap', plan) === 'cheap',
    'automatic floor is standard; explicit cheap remains respected',
  ));
  return trials;
}
